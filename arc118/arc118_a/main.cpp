...
int main(){
    ...
    auto lengthOfCycle = 100+t;
    auto numberSkippedPerCycle = skipped.size();
    auto cycleNIn = (n-1)/numberSkippedPerCycle;
    auto startingPoint = cycleNIn*lengthOfCycle;
    auto skippedByStarting = cycleNIn*numberSkippedPerCycle;
    int i = 0;
    dbg(lengthOfCycle);
    dbg(numberSkippedPerCycle);
    dbg(cycleNIn);
    dbg(startingPoint);
    dbg(skippedByStarting);
    
    dbg(skipped);
    i = n-skippedByStarting-1;
    cout << startingPoint+skipped[i] << endl;
    return 0;
}
