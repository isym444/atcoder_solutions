...
std::vector<auto> solve(int N, const std::vector<long long> &A, const std::vector<long long> &B, const std::vector<long long> &U, const std::vector<long long> &V) {
...
int main() {
    ...
    int N;
    std::cin >> N;
    std::vector<long long> A(N), B(N), U(N - 1), V(N - 1);
    REP (i, N) {
        std::cin >> A[i] >> B[i];
    }
    REP (i, N - 1) {
