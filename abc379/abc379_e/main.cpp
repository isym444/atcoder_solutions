...
long long repeatDigit(int digit, int x) {
    // Input validation
    if (digit < 0 || digit > 9 || x < 0) {
        throw std::invalid_argument("Digit must be between 0 and 9, and x must be non-negative.");
    }

    long long result = 0;
    for (int i = 0; i < x; ++i) {
        // Multiply result by 10 using bit shifts and additions
        result = ((result << 3) + (result << 1)) + digit; // Equivalent to result = result * 10 + digit

        // Check for overflow
        if (result < 0 || result > std::numeric_limits<long long>::max() - digit) {
