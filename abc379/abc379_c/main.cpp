...
int main(){
    ...
    sort(all(XA),[](pair<ll,ll> aa, pair<ll,ll> bb){
        return aa.first < bb.first;
    });
    dbg(XA);
    //if first cell doesn't contain a stone then impossible
    if(XA[0].first!=1){
        cout << -1 << endl;
        return 0;
    }
    ...
    foi(0,M){
        ...
        if(i==M-1){
            ...
            ll numBlank = N-XA[i].first;
            if(numBlank!=XA[i].second-1){
                cout << -1 << endl;
                return 0;
            }
            if(numBlank==0) continue;
            ans+=sum(1,numBlank);
            continue;
        }
        ll numBlank = XA[i+1].first-XA[i].first-1;
        if(numBlank>XA[i].second-1){
            cout << -1 << endl;
            return 0;
        }
        XA[i+1].second+=XA[i].second-1-numBlank;
        ans+=(XA[i].second-1-numBlank)*(XA[i+1].first-XA[i].first);
        if(numBlank==0) continue;
        ans+=sum(1,numBlank);
