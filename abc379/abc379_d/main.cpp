...
int main(){
    ...
    foi(0,Q){
        ...
        if(a==2){
            ll b;
            cin >> b;
            vector<ll> v(ms.begin(), ms.end());
            foi(0,v.size()){
                v[i]=v[i]+=b;
            }
            multiset<ll> mset(v.begin(), v.end());
            ms=mset;
            continue;
        }
        if(a==3){
            ll b;
            cin >> b;
            vll v(ms.begin(), ms.end());
            ll targ = indlb(v,b);
            ll harvested = v.size()-targ+1;
            cout << harvested-1 << endl;
            // ans+=harvested;
            multiset<ll> mset;
            mset.insert(v.begin(), v.begin() + targ);
            ms = mset;
            continue;
        }
    }
