...
int main(){
    ...
    ll inittotalapples = accumulate(A.begin(), A.end(), 0LL);
    auto SA = A;
    sort(SA.begin(), SA.end());

    auto ans = A;
    ll curRemainingTotalApples = inittotalapples;
    ll curTotalApplesRemovedSoFar = 0;
    ll curTotalCyclesRemoved = 0;
    bool broken = false;

    for (ll i = 0; i < N; i++){
        ll remainingApplesInOneCycle = N - i;
        ll numberOfCyclesToTryRemove = SA[i] - curTotalCyclesRemoved;

        ll potentialRemainingTotalApples = curRemainingTotalApples - remainingApplesInOneCycle * numberOfCyclesToTryRemove;
        ll potentialTotalApplesRemovedSoFar = inittotalapples - potentialRemainingTotalApples;

        if(potentialTotalApplesRemovedSoFar <= K){
            curRemainingTotalApples = potentialRemainingTotalApples;
            curTotalApplesRemovedSoFar = potentialTotalApplesRemovedSoFar;
            curTotalCyclesRemoved += numberOfCyclesToTryRemove;
        } else {
            ll applesToRemoveFromEachRemaining = (K - curTotalApplesRemovedSoFar) / (N - i);
            ll remaining = (K - curTotalApplesRemovedSoFar) % (N - i);

            for (ll j = 0; j < N; j++){
                if(ans[j] > curTotalCyclesRemoved){
                    ans[j] -= curTotalCyclesRemoved;
                    ans[j] -= applesToRemoveFromEachRemaining;
                    if(ans[j] < 0) ans[j] = 0;
                } else {
                    ans[j] = 0;
                }
    ...
    if(!broken){
        // Update ans after loop completion
        for(ll j = 0; j < N; j++){
            ans[j] -= curTotalCyclesRemoved;
            if(ans[j] < 0) ans[j] = 0;
        }
...
//     ll breaker=0;
//     foi(0,N){
//         //remove SA[i] number of cycles
//         ll remainingApplesInOneCycle=N-i;
//         ll numberOfCyclesToTryRemove=SA[i];
//         ll potentialRemainingTotalApples=curRemainingTotalApples-remainingApplesInOneCycle*numberOfCyclesToTryRemove;
//         ll potentialTotalApplesRemovedSoFar=inittotalapples-potentialRemainingTotalApples;
//         if(potentialTotalApplesRemovedSoFar<=K){
//             curRemainingTotalApples=potentialRemainingTotalApples;
...
//             curTotalCyclesRemoved+=numberOfCyclesToTryRemove;
//         }else{
//             dbg(curRemainingTotalApples);
//             dbg(curTotalApplesRemovedSoFar);
//             ll applesToRemoveFromEachRemaining = floordiv(K-curTotalApplesRemovedSoFar,N-i);
//             dbg(applesToRemoveFromEachRemaining);
//             foj(0,N){
//                 if(curTotalCyclesRemoved>=ans[j]) ans[j]=0;
//                 else{
//                     ans[j]-=curTotalCyclesRemoved;
//                     ans[j]-=applesToRemoveFromEachRemaining;
//                 }
//             }
//             // curTotalApplesRemovedSoFar-=applesToRemoveFromEachRemaining*(N-i);
//             dbg(ans);
//             ll finalremainingApplesToRemove = (K-curTotalApplesRemovedSoFar)%(N-i);
//             dbg(finalremainingApplesToRemove);
//             foj(0,N){
//                 if(ans[j]==0) continue;
//                 if(finalremainingApplesToRemove==0) break;
//                 ans[j]--;
//                 finalremainingApplesToRemove--;
//                 if(finalremainingApplesToRemove==0) break;
//             }
//             break;
//         }
