...
vector<pair<ll,ll>> ha; //height_age

long long solve(int N, std::string S, const std::vector<long long> &W) {
...
int main() {
    ...
    //if have tie, want children before adults
    sort(all(ha),[](pair<ll,ll> a, pair<ll,ll> b){
        if(a.first==b.first){
    ...
    foi(0,N){
        ...
        if(ha[i].second==1) ans--;
        if(ha[i].second==0) ans++;
        // dbg(ans);
        prev=ha[i].first;
    }
    dbg(S.length());
