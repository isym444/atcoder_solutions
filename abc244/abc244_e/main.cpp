...
// Define mint for modular arithmetic with modulus 998244353
using mint = modint998244353;
...
//v, k, X appeared even or odd times
mint dp[2005][2005][2];
...
// int main() {
//   int n, m, K, s, t, x;
//     dp.resize(5, vector<vector<ll>>(5, vector<ll>(2)));
// //   dp.resize()
//   // Read input values
//   cin >> n >> m >> K >> s >> t >> x;
//   --s; --t; --x; // Adjust to zero-based indices
  
//   vector<vector<int>> to(n); // Adjacency list for the graph
//   rep(i,m) {    
//     int a, b;
//     cin >> a >> b;
//     --a; --b; // Adjust to zero-based indices
//     to[a].push_back(b);
//     to[b].push_back(a);
//   }

//   // Initial condition: start at vertex s with even occurrences of x
//   dp[0][s][0] = 1;
  
//   // DP transition
//   rep(k,K) rep(v,n) rep(i,2) {
//     dbg(v);
//     fx(dp){
//         dbg(x);
//     }
//     cerr << endl;
//     for (int u : to[v]) {
//       int nk = k + 1; // Next step
//       int nv = u; // Next vertex
//       int ni = i; // Next parity state
//       if (u == x) ni ^= 1; // Toggle parity if u is x
//       dp[nk][nv][ni] += dp[k][v][i]; // Update DP table
//     }
//   }

//   // The answer is the number of ways to reach vertex t at step K with even occurrences of x
//   fx(dp){
//     dbg(x);
//   }
//   ll ans = dp[K][t][0];
//   cout << ans+1 << endl; // Output the result
