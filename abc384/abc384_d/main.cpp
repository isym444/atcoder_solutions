...
bool targetSumExists(const vll& arr, ll target) {
    int n = arr.size();
    ll total_sum = 0;
    for (ll num : arr) total_sum += num;
    ...
    // Traverse the array twice to simulate wrapping
    for (int i = 0; i < 2 * n; i++) {
        prefix_sum += arr[i % n];

        // Check if prefix_sum - target exists in the set
        if (seen.count(prefix_sum - target)) {
            return true;
        }

        // Update the set only during the first traversal
        if (i < n) {
