...
int main() {
    ...
    foi(0,S.length()){
        ...
        else if(i>indexes[counter]&&S[i]=='L'){
            lc++;
        }
        else{
            ans[indexes[counter]]=ceildiv(rc,2)+lc-ceildiv(lc,2);
            ans[indexes[counter]+1]=rc-ceildiv(rc,2)+ceildiv(lc,2);
            rc=0;
            lc=0;
            counter++;
            i--;
