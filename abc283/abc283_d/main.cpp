...
void solve(std::string S) {
    ...
    fx(S){
        if(x=='('){
            numineach.pb(pon.size());
        }
        else if(x==')'){
            ll tbr = pon.size()-numineach[numineach.size()-1];
            /* cerr << "to be removed" << tbr << endl;
            cerr << "pon before removal: " << pon << endl; */
            foi(0,tbr){
                quickchecker[pon.back()]=0;
                pon.pop_back();
                //cerr << "hello" << endl;
            }
            //cerr << "pon after removal: " << pon << endl;
            numineach.pop_back();
        }
        else{
            if(quickchecker[x]==1){
