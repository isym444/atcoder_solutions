...
void solve(const std::vector<vector<ll>> &a) {
    // TODO: edit here
    ...
    for(int m=0;m<9;m+=3){
        for(int i = 0; i<9; i+=3){
            ...
            for(int j=0+i;j<3+i;j++){
                for(int k=0+m;k<3+m;k++){
                    if(seen.find(a[j][k])!=seen.end()){
                        cout << "No" << endl;
                        return;
                    }
                    seen.insert(a[j][k]);
...
int main() {
    ...
    // TODO: edit here
    std::vector<vector<ll>> a(9,vector<ll>(9,0));
    foi(0,9) foj(0,9) cin >> a[i][j];
    solve(a);
