...
void solve(int N, const std::vector<long long> &x, const std::vector<long long> &y) {
    /* vis.assign(n+1, false);
    g.assign(n+1, vector<ll>());
    wg.assign(n + 1, vector<pair<ll,ll>>());
    parent.assign(n+1, -1); */
    //x is power, y is cost
    vector<tuple<ll,ll,ll>> pc;
    foi(0,N){
        pc.push_back(mt(x[i],y[i],i+1));
    }
    sort(all(pc),[](tuple<ll,ll,ll> a, tuple<ll,ll,ll> b){
