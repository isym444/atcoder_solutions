...
long long solve(int N, const std::vector<long long> &T, const std::vector<long long> &D, const std::vector<long long> &X) {
...
int main() {
    ...
    int N;
    std::cin >> N;
    std::vector<long long> T(N), D(N), X(N);
    REP (i, N) {
        std::cin >> T[i] >> D[i] >> X[i];
    }
    auto ans = solve(N, T, D, X);
    std::cout << ans << '\n';
