...
bool solve(long long a, int b, const std::vector<vector<char>> &c) {
    ...
    foi(0,a) foj(0,b){
        if(c[i][j]=='#'){
            if(c[i-1][j]!='#'&&c[i+1][j]!='#'&&c[i][j-1]!='#'&&c[i][j+1]!='#') return false;
        }
    }
    return true;
}
...
int main() {
    ...
    int b;
    std::cin >> a >> b;
    std::vector<vector<char>> c(55,vector<char>(55,'.'));
    for(int i = 0; i<a; i++){
        for(int j=0; j<b; j++){
            cin >> c[i+1][j+1];
        }
    }
    //for(auto x:c) cerr << x << endl;
