...
int main(){
    // iterate once through all positions of targets
    // map row position (i) -> number of targets &&&&&& keep track of max number of targets, row position
    // iterate once through all positions of targets
    // map column position (j) -> number of targets &&&&&& keep track of max number of targets, column position
    // iterate once through all positions of targets
    // map pair(i,j) of target -> 1 i.e. tells you whether a particular square has a target or not (to help avoid double counting if at centre of explosion)
    // check if need to subtract 1 from answer if double counted centre of cross
    ll H,W,M;
    cin >> H >> W >> M;
    map<ll,ll> rowPos;
    pair<ll,ll> maxRow=mp(-1,-INF);
    map<ll,ll> colPos;
    pair<ll,ll> maxCol=mp(-1,-INF);
    map<pair<ll,ll>,ll> squareWithTarget;
    ...
    foi(0,M){
        ...
        all.pb(mp(h,w));
        rowPos[h]++;
        if(rowPos[h]>maxRow.first){
            maxRow=mp(rowPos[h],h);
        }
        colPos[w]++;
        if(colPos[w]>maxCol.first){
            maxCol=mp(colPos[w],w);
        }
        squareWithTarget[mp(h,w)]=1;
    }

    vll potRows;
    fx(rowPos){
        if(x.second==maxRow.first){
            potRows.pb(x.first);
        }
    }
    vll potCols;
    fx(colPos){
        if(x.second==maxCol.first){
            potCols.pb(x.first);
        }
    }
    ...
    // fx(potRows){
    //     for(auto y:potCols){
    //         if(squareWithTarget[mp(x,y)]==0) checker = 1;
    //     }
    // }

    // flip on its head and instead check whether for all squares with target, if any exist at crosshair btwn potRows and potCols
    // if the number is not equal to the number of crosshair combos then can make checker = 1
    ...
    fx(squareWithTarget){
        auto [h,w] = x.first;
        if(binary_search(all(potRows),h)&&binary_search(all(potCols),w)){
            temp++;
        }
    }

    if(potRows.size()*potCols.size()!=temp){
        checker = 1;
    }
    ...
    ans+=maxRow.first;
    ans+=maxCol.first;
    // if(squareWithTarget[mp(maxRow.second,maxCol.second)]==1) ans--;
    if(checker==0) ans--;
    cout << ans << endl;
    return 0;
}