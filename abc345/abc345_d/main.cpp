...
bool solve(int N, long long H, long long W, const std::vector<long long> &A, const std::vector<long long> &B) {
    ...
    for(int i = 0; i < (1 << areas.size()); i++) {
        ...
        for(int j = 0; j < areas.size(); j++) {
            if(i & (1 << j)) { // Check if the j-th bit of i is set
                sum += areas[j]; // Include areas[j] in the current combination
            }
        }
        cerr << "Combination for i = " << i << ": sum = " << sum << endl; // Debug: view each combination's sum

        // Here, you can check if the sum matches your criteria, e.g., equals target
        if(sum == target) {
            return true; // Found a combination that matches the target
        }
    }
    return false;
}
