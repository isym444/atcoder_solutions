...
long long solve(int N, const std::vector<long long> &u, const std::vector<long long> &v, const std::vector<long long> &A) {
    // TODO: edit here
}
...
int main() {
    ...
    int N;
    std::cin >> N;
    std::vector<long long> u(N - 1), v(N - 1), A(N);
    REP (i, N - 1) {
        std::cin >> u[i] >> v[i];
    }
    REP (i, N) {
