...
const long double PI = 3.141592653589793238462643383279502884L; // Higher precision Pi
...
double solve(double a, double b, double x) {
    ...
    dbg(x);
    if(x>=b*H/2){
        long double ya = H*b/2.0L;
        long double A2 = x-ya;
        long double L1 = 2.0L*A2/b;
        long double L2 = H-L1;
        // double L2 = (2*H*b-2*x)/b;
        return atan2(L2,b)*(180/acos(-1));
    }
    else{
        double w = x*2/H;
        return atan2(H,w)*(180/acos(-1));
    }
}
