...
int main(){
    // operation: all Ai into Aj
    // xor of all A after performing operation 0 times
    // naive:
    // get all pairs of i,j and perform operation for all non-empty Ai -> xor of all A AND don't have to do foi foj, j can be from i+1 to N-1
    // continue until only one non-empty A[i]
    // Can speed up xor of all A by using segment tree
    // Can speed up further using memoization
    // OBSERVATIONS TO SPEED UP
    // Is A xor B unique? -> NO, which means you cannot use number of different combinations of arrangements in bags as proxy for xor of whole array
    // N is only 12 so ?brute force will work
    ...
    cin >> A;
    set<ll> ans;
    // Surely, can just make new vector<vll> with all possible merges at each step
    // Keep going until vector<vll> is empty
    vvll comp;

    auto recur = [&](auto recur, vll prev)->void{
        foi(0,N){
            vll temp=prev;
            foj(i+1,N){
                // always put into j rather than i
                if(i==0) continue;
