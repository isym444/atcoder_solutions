...
int main(){
    // find top left & bottom right corners of the rectangle -> NO left most, up most, right most, down most points
    // if there are any white points within rectangle then cout no
    ll H,W;
    cin >> H >> W;
    vector<string> G(H);
    foi(0,H){
    ...
    foi(0,H){
        foj(0,W){
            if(G[i][j]=='#'){
                leftMost = min(leftMost,(ll)j);
                upMost = min(upMost,(ll)i);
                rightMost = max(rightMost,(ll)j);
                downMost = max(downMost,(ll)i);
            }
        }
    }
    dbg(mt(leftMost,upMost,rightMost,downMost));
    foi(upMost,downMost+1){
        foj(leftMost,rightMost+1){
            if(G[i][j]=='.'){
                cout << "No" << endl;
                return 0;
            }
