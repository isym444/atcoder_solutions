...
long long solve(int N, long long X, const std::vector<long long> &V, const std::vector<long long> &A, const std::vector<long long> &C) {
    // TODO: edit here
}
...
int main() {
    ...
    long long X;
    std::cin >> N;
    std::vector<long long> V(N), A(N), C(N);
    std::cin >> X;
    REP (i, N) {
        std::cin >> V[i] >> A[i] >> C[i];
    }
