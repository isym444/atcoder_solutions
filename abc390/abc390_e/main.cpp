...
int main(){
    ...
    vector<vector<tuple<ll,ll,ll>>> dp(X+1,vector<tuple<ll,ll,ll>>(N+1,{0,0,0}));

    ll ans = 0;

    // initially manually from 0,0
    vvll visited(X+1,vll(N+1,0));
    visited[0][0] = 1;
    ...
    foi(0,N){
        foj(0,X){
            if (visited[j][i]) {
                // if we eat
                if (j + C[i] <= X) {
                    tuple<ll, ll, ll> newtriple = dp[j][i];
                    if (V[i] - 1 == 0) {
                        get<0>(newtriple) = get<0>(dp[j][i]) + A[i];
                    } else if (V[i] - 1 == 1) {
                        get<1>(newtriple) = get<1>(dp[j][i]) + A[i];
                    } else if (V[i] - 1 == 2) {
                        get<2>(newtriple) = get<2>(dp[j][i]) + A[i];
                    }
                    if(i==0 && j==0) dbg(newtriple);

                    // if (min({get<0>(newtriple), get<1>(newtriple), get<2>(newtriple)}) >
                    //     min({get<0>(dp[j + C[i]][i + 1]), get<1>(dp[j + C[i]][i + 1]), get<2>(dp[j + C[i]][i + 1])})) {
                    // }
                    dp[j + C[i]][i + 1] = newtriple;
                    ans = max(ans, min({get<0>(dp[j + C[i]][i + 1]), get<1>(dp[j + C[i]][i + 1]), get<2>(dp[j + C[i]][i + 1])}));
                    visited[j + C[i]][i + 1] = 1;
                }

                // if we don't eat
                // if (min({get<0>(dp[j][i]), get<1>(dp[j][i]), get<2>(dp[j][i])}) >
                //     min({get<0>(dp[j][i + 1]), get<1>(dp[j][i + 1]), get<2>(dp[j][i + 1])})) {
                // }
                dp[j][i + 1] = dp[j][i];
                ans = max(ans, min({get<0>(dp[j][i + 1]), get<1>(dp[j][i + 1]), get<2>(dp[j][i + 1])}));
                visited[j][i + 1] = 1;
            }
        }
    }
