...
string solve(long long N, int M, const std::vector<long long> &s, const std::vector<long long> &c) {
    // TODO: edit here
    ...
    for(int i = 0; i<M; i++){
        /* cerr << i << endl;
        cerr << s << " " << c << " " << ans << endl;
        cerr << ans[s[i]-1] << " " << c[i] << " " << ans << endl; */
        if(ans[s[i]-1]!=-1&&ans[s[i]-1]!=c[i]) return "-1";
        else ans[s[i]-1]=c[i];
    }
    //cerr << ans << endl;
    if(N==1){
...
int main() {
    ...
    int M;
    std::cin >> N >> M;
    std::vector<long long> s(M), c(M);
    REP (i, M) {
        std::cin >> s[i] >> c[i];
    }
    string p = solve(N, M, s, c);
    //cerr << p;
    cout << p;
