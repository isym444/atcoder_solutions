...
long long solve(long long X, long long A, long long D, long long N) {
    ...
    if(true){
        cerr << "hello" << endl;
        if(MOD((X-A),D)==0) return 0;
        else{
            ll temp = floordiv((X-A),D); //h fix negative numbers logic
            // cerr << temp << endl;
            return (min(abs(X-(A+temp*D)),abs((A+(D*(temp+1)))-X)));
        }
    }
}
