...
long long solve(int N, long long C, const std::vector<long long> &s, const std::vector<long long> &t, const std::vector<long long> &c) {
    // TODO: edit here
}
...
int main() {
    ...
    long long C;
    std::cin >> N;
    std::vector<long long> s(N), t(N), c(N);
    std::cin >> C;
    REP (i, N) {
        std::cin >> s[i] >> t[i] >> c[i];
    }
    auto ans = solve(N, C, s, t, c);
    std::cout << ans << '\n';
