...
int main() {
    ...
    std::cin >> S;
    foi(0,N){
        loc locobj(X[i],Y[i],S[i]);
        xy.pb(locobj);
    }
    /* for(auto x : xy){
        cerr << x.x << " " << x.y << " " << x.dir << endl;
    } */
    // Sorting by y first, then by x
    std::sort(xy.begin(), xy.end(), [](const loc &a, const loc &b) {
        if (a.y == b.y) {
            return a.x < b.x; // Sort by x if y is the same
        }
        return a.y < b.y; // Otherwise, sort by y
    ...
    for(auto x:temp){
        ...
        ll lchecker=0;
        for(auto p:x.second){
            if(rchecker&&p=='L') lchecker=1;
            if(p=='R') rchecker=1;
        }
        if(rchecker&&lchecker){
            cout << "Yes";
            return 0;
        }
    }
