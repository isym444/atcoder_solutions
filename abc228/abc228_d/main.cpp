...
template <class T> int indlb(const std::set<T> &v, const T &x) { return std::distance(v.begin(), std::lower_bound(v.begin(), v.end(), x)); }
...
int main() {
    ...
    REP (i, Q) {
        ...
        if(t==1){
            ...
            dbg(x);
            h=h%N;
            // for (auto it = a.begin(), end = std::next(a.begin(), std::min<size_t>(5, a.size())); it != end; ++it) dbg(*it);
            auto it = a.lower_bound(h);
            if(it==a.end()){
                it = a.begin();
                // h = indlb(a,h);
            }
