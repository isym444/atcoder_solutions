...
auto solve(long long a, int b, const std::vector<long long> &c, const std::vector<long long> &d, const std::vector<long long> &e, long long f) {
...
int main() {
    ...
    long long f;
    std::cin >> a >> b;
    std::vector<long long> c(b), d(b), e(b);
    REP (i, b) {
        std::cin >> c[i] >> d[i] >> e[i];
    }
    std::cin >> f;
