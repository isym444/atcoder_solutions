...
int main(){
    ...
    map<vector<ll>,ll> dp;
    vll init(K,0);
    //costs 0 to get to state where every parameter is 0
    dp[init]=0;
    foi(0,N){
        ll C;
        cin >> C;
        vll A(K);
        cin >> A;
        //for every current state already visited (i.e. in dp), apply A and C, storing C only if beats current value (cheaper)
        auto newdp = dp;
        for(auto [state, cost] : dp){
            auto newState = state;
            foj(0,K){
                newState[j]=min(newState[j]+A[j],P);
            }
            if(newdp.count(newState)){
                newdp[newState]=min(cost+C,newdp[newState]);
            }
            else{
                newdp[newState]=cost+C;
            }
        }
        dp=newdp;
        // dbg(dp);
