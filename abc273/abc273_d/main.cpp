...
int main() {
    ...
    foi(0,Q){
        ...
        cin >> d >> l;
        if(d=='L'){
            ll temp = indlb(rowWalls[curT.first],curT.second);
            dbg(temp);
            temp--;
            // temp=max((ll)0,temp);
            if(temp==-1){
                curT.second=max((ll)0,curT.second-l);
            }
            else{
                curT.second=max(rowWalls[curT.first][temp]+1,curT.second-l);
            }
            // temp=min(W,temp);
        }
        if(d=='R'){
            ll temp = indlb(rowWalls[curT.first],curT.second);
            temp=max((ll)0,temp);
            // temp=min((ll)rowWalls[curT.first].size(),temp);
            if(temp>=(ll)rowWalls[curT.first].size()){
                curT.second=min(W-1,curT.second+l);
            }
            else{
                curT.second=min(rowWalls[curT.first][temp]-1,curT.second+l);
            }
        }
        if(d=='U'){
            ll temp = indlb(colWalls[curT.second],curT.first);
            dbg(temp);
            temp--;
            // temp=max((ll)0,temp);
            if(temp==-1){
                curT.first=max((ll)0,curT.first-l);
            }
            else{
                curT.first=max(colWalls[curT.second][temp]+1,curT.first-l);
            }
            // temp=min(H,temp);
        }
        if(d=='D'){
            ll temp = indlb(colWalls[curT.second],curT.first);
            temp=max((ll)0,temp);
            // temp=min((ll)colWalls[curT.first].size(),temp);
            if(temp>=(ll)colWalls[curT.second].size()){
                curT.first=min(H-1,curT.first+l);
            }
            else{
                curT.first=min(colWalls[curT.second][temp]-1,curT.first+l);
            }
        }
        dbg(i);
        cout << curT.first+1 << " " << curT.second+1 << endl;
    }
