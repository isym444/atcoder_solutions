...
long long solve(long long N, int M, const std::vector<long long> &l, const std::vector<long long> &r, const std::vector<long long> &a) {
...
int main() {
    ...
    int M;
    std::cin >> N >> M;
    std::vector<long long> l(M), r(M), a(M);
    REP (i, M) {
        std::cin >> l[i] >> r[i] >> a[i];
    }
