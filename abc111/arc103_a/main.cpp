...
int main(){
    ...
    map<ll,ll> even;
    map<ll,ll> odd;
    pair<ll,ll> maxeven=mp(0,0);//number of numbers , actual number
    pair<ll,ll> maxodd=mp(0,0);//number of numbers , actual number
    // ll maxeven=0; //don't actually need to track which number it is
    // ll maxodd=0;
    foi(0,N){
        tot[V[i]]++;
        if(i%2==0){
            even[V[i]]++;
            // maxeven=max(maxeven,even[V[i]]);
            if(maxeven.first<even[V[i]]){
                maxeven=mp(even[V[i]],V[i]);
            }

        }
        else{
            // maxodd=max(maxodd,odd[V[i]]);
            odd[V[i]]++;
            if(maxodd.first<odd[V[i]]){
                maxodd=mp(odd[V[i]],V[i]);
            }
        }
    }
    ll ans=0;
    ...
    if(maxeven.second==maxodd.second){
        even.clear();
        odd.clear();
        pair<ll,ll> d0=mp(0,0);
        pair<ll,ll> d1=mp(0,0);
        foi(0,N){
            if(i%2==0){
                if(V[i]==maxeven.second) continue;
                even[V[i]]++;
                if(d0.first<even[V[i]]){
                    d0=mp(even[V[i]],V[i]);
                }
            }
            else{
                if(V[i]==maxodd.second) continue;
                odd[V[i]]++;
                if(d1.first<odd[V[i]]){
                    d1=mp(odd[V[i]],V[i]);
                }
        ...
        ans=min(N-maxeven.first-d1.first,N-maxodd.first-d0.first);
        // ans+=min(d0.first,d1.first);
        cout << ans << endl;
        return 0;
    }
    // if(maxeven.second==maxodd.second){
    ...
    // }
    dbg(maxeven.first);
    ans+=(N/2)-maxeven.first;
    ans+=(N/2)-maxodd.first;
    cout << ans << endl;
    return 0;
}