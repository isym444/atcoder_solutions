...
int main(){
    ...
    ll ans = 0;
    ll bpointer=0;
    while(bpointer<N && B[bpointer]>=0){
        ans+=B[bpointer];
        bpointer++;
    }
    dbg(ans);
    ll wpointer=0;
    while(wpointer<M && wpointer<bpointer && W[wpointer]>=0){
        ans+=W[wpointer];
        wpointer++;
    }
    dbg(ans);
    while(wpointer<M && bpointer<N && (W[wpointer]+B[bpointer])>=0){
        ans+=W[wpointer]+B[bpointer];
        wpointer++;
        bpointer++;
    }
    cout << ans << endl;
