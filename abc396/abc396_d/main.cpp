...
long long binaryToDecimal(const std::string& binaryString) {
    return std::stoll(binaryString, nullptr, 2);
}
...
int main(){
    ...
    auto dfs = [&](auto dfs, ll v, string x, vector<ll> visited) -> void {
        if(v==N-1){
            ans = min(ans, binaryToDecimal(x));
            return;
        }
        if(visited[v]){
            return;
        ...
        for(auto [u, w] : G[v]) {
            if(!visited[u]){ {
                dfs(dfs, u, string_xor(x, w), visited);
                }
            }
        }
    };
