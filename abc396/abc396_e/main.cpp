...
int main(){
    ...
    vll A(N,INF);
    vll connectedComponent;
    ll breaker=0;
    auto dfs = [&](auto& dfs, int v, ll val)->void{
        A[v] = val;
        connectedComponent.pb(v);
        for(auto [u, ew]: G[v]){
            if(A[u]!=INF){
                if(A[u]!=(val^ew)){
                    if(!breaker) cout << -1 << endl;
                    exit(0);
                }
                continue;
            }
    ...
    foi(0,N){
        ...
        ll mask=0;
        foj(0,30){
            ll onesInBitColumn=0;
            fok(0,connectedComponent.size()){
                if(A[connectedComponent[k]]&(1<<j)) onesInBitColumn++;
            }
            if(onesInBitColumn>connectedComponent.size()-onesInBitColumn){
                mask|=(1<<j);
            }
        }
        foj(0,connectedComponent.size()){
            A[connectedComponent[j]]^=mask;
        }
    }
    fx(A) cout << x << " ";
    cout << endl;
