...
int main(){
    ...
    foi(1,10){
        ...
        foj(1,10){
            ...
            ll Ascore=0;
            fok(1,10){
                Tscore+=k*pow(10,ST[k]);
                Ascore+=k*pow(10,SA[k]);
            }
            if(Tscore>Ascore){
                // Twin++;
                if(i==j){
                    Twin+=calc[i]*(calc[i]-1);
                }else{
                    Twin+=calc[i]*calc[j];
                }
            }else{
    ...
    long double ans=0;
    dbg(mp(Twin,Awin));
    ans=(long double)(Twin)/(long double)(Twin+Awin);
    ll SS = 9*K-8;
    // ans=(long double)(Twin)/(long double)(SS*(SS-1));
