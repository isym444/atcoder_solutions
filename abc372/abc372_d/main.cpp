...
int main(){
    ...
    // s.push_front(H[N-1]);
    for(int i = N-1; i>=0; i--){
        while(!s.empty()&&H[i]>s.front()) s.pop_front();
        s.push_front(H[i]);
        dbg(s);
        if(i>0) ans[i-1]=s.size();
    }
    dbg(ans);
    fx(ans){
        cout << x << " ";
...
// int main() {
//     int n;
//     cin >> n;  // Read the number of buildings
//     vector<int> h(n);  // Vector to store the heights of the buildings
//     vector<int> ans(n, 0);  // Initialize answer array with 0s
//     vector<int> stc;  // Stack to keep track of visible building indices
...
//     // Traverse the buildings from the second last to the first
//     for (int i = n - 2; i >= 0; --i) {
//         // dbg(h[stc]);
//         // Pop elements from the stack while they are shorter than the next building
//         while (!stc.empty() && h[stc.back()] < h[i + 1]) {
//             stc.pop_back();
//         }
//         // Add the next building's index to the stack
//         stc.push_back(i + 1);
//         // The number of visible buildings from building i is the size of the stack
//         dbg(stc);
//         fx(stc){
//             dbg(h[x]);
//         }
