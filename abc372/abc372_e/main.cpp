...
int main(){
    ...
    foi(0,Q){
        ...
        if(type==1){
            ...
            dbg(mt(ar,br,newr));
            if(newr==ar){
                ch[ar].insert(all(ch[br]));
                ch[br].clear();
            }
            else{
                ch[br].insert(all(ch[ar]));
                ch[ar].clear();
            }
            dbg(ch);
        }
...
//     // Initialize Union-Find for N vertices (1-indexed)
//     atcoder::dsu uf(N + 1);

//     // Member tracking: vector to store sorted members of each component
//     vector<set<int>> member(N + 1);

//     // Initially, each vertex is its own component, so we add them
//     for (int i = 1; i <= N; ++i) {
//         member[i].insert(i);
//     }
...
//             // If u and v are already in the same component, no need to merge
//             if (uf.same(u, v)) continue;

//             // Merge components containing u and v
//             int root_u = uf.leader(u);
//             int root_v = uf.leader(v);

//             uf.merge(u, v);

//             // After merging, decide which component will become the new root
//             int new_root = uf.leader(u);  // Could be root_u or root_v

//             if (new_root == root_u) {
//                 // Merge the members of component v into u's component
//                 member[root_u].insert(member[root_v].begin(), member[root_v].end());
//                 member[root_v].clear();  // Clear the old set
//             } else {
//                 // Merge the members of component u into v's component
//                 member[root_v].insert(member[root_u].begin(), member[root_u].end());
//                 member[root_u].clear();  // Clear the old set
//             }

//         } else if (type == 2) {
//             // Type 2 query: Find the k-th largest vertex in the component of v
//             int v, k;
//             cin >> v >> k;

//             int root_v = uf.leader(v);  // Find the root of the component containing v

//             if (member[root_v].size() < k) {
//                 cout << -1 << endl;  // Not enough vertices in the component
//             } else {
//                 // We need to retrieve the k-th largest vertex from the sorted set
//                 auto it = member[root_v].rbegin();  // Reverse iterator to get largest elements
//                 advance(it, k - 1);  // Move to the k-th largest element
//                 cout << *it << endl;
//             }
//         }
//     }
