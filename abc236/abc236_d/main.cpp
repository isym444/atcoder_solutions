...
int main(){
    ...
    auto dfs = [&](auto dfs, vll pairedyet, ll curxor)->void{
        ll firstunpaired = -1;
        foi(0,2*N){
            if(pairedyet[i]==0){
                firstunpaired=i;
        ...
        if(firstunpaired==-1){
            //then everyone is paired
            ans = max(ans,curxor);
            return;
        }
        foi(0,2*N){
            if(pairedyet[i]==0&&i!=firstunpaired){
                pairedyet[i]=1;
                pairedyet[firstunpaired]=1;
                ll argxor = curxor^(A[min(firstunpaired,(ll)i)][max(firstunpaired,(ll)i)]);
                dbg(mt(min(firstunpaired,(ll)i),max(firstunpaired,(ll)i)));
                dbg(A[min(firstunpaired,(ll)i)][max(firstunpaired,(ll)i)]);
                dfs(dfs, pairedyet, argxor);
                pairedyet[i]=0;
                pairedyet[firstunpaired]=0;
            }
        }
    };

    vll allunpaired(2*N,0);

    dfs(dfs, allunpaired, 0);
