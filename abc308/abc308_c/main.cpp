...
struct ReverseComparator {
    bool operator()(const std::pair<ll, long long>& a, const std::pair<ll, long long>& b) const {
        if (a.first == b.first) {
            return a.second < b.second; // Tie-breaker: Use the second element for comparison
        }
        return a.first > b.first; // Primary: Sort by the first element in reverse order
    }
};
...
int main() {
    ...
    stable_sort(AB.begin(),AB.end(),[](tuple<ll,ll,ll> a, tuple<ll,ll,ll> b){
        ll a1 = get<0>(a);
        ll a2 = get<0>(b);
        // ll a2 = b.get(0);
        // ll b1 = a.get(1);
