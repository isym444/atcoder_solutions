...
ll H, W;
vector<char> nextchar = {'s','n','u','k','e'};
vector<char> nnext;

vvll vvisited;

void ddfs(ll x, ll y, vvll& vvisited, ll mazelength){
    ...
    mazelength++;
    foi(0,4){
            if(isvalid(x+dx_wasd[i],H,y+dy_wasd[i],W)){
                // visited[x+dx_wasd[i]][y+dy_wasd[j]]=1;
                ll nextx = x+dx_wasd[i];
                ll nexty = y+dy_wasd[i];
                char nnextchar = S[nextx][nexty];
                char nextexpectedchar = nextchar[(mazelength-0)%5];
                // char nextexpectedchar = nnext[S[x][y]];
                // cerr << "next character: " << nnextchar << " next expected char: " << nextexpectedchar << endl;
                if(nnextchar==nextexpectedchar&&!vvisited[nextx][nexty]){
                    ddfs(nextx, nexty, vvisited, mazelength);
                }
            }
    }
    // visited[x][y]=0;
...
bool solve(int H, long long W, const std::vector<std::string> &S) {
    /* vis.assign(n+1, false);
    g.assign(n+1, vector<ll>());
    wg.assign(n + 1, vector<pair<ll,ll>>());
    parent.assign(n+1, -1); */
    vvisited=vvll(H,vll(W,0));
    ddfs(0,0,vvisited, 0);
    // cerr << vvisited << endl;
