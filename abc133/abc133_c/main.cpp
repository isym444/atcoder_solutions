...
long long solve(long long L, long long R) {
    ...
    if(R-L>=2019){
        return 0;
        // if(MOD(L,2019)==0 || MOD(R,2019)==0) return 0;
        
    }
    else if(MOD(L,2019)>MOD(R,2019)){
        return 0;
    }
    else{
        // ll lor = 0;
        // if(MOD(R,2019)<MOD(L,2019)) lor=1;
        // ll temp = min(MOD(R,2019), MOD(L,2019));
        // return temp*(temp+1);
        ll ans = INF;
        foi(MOD(L,2019),MOD(R,2019)+1){
            foj(i+1,MOD(R,2019)+1){
                if(i==j) continue;
                ans=min(ans,(ll)MOD(i*j,2019));
            }
        }
        return ans;
