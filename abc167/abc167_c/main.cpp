...
int main() {
    ...
    for(int i = 0; i < 1<<n; i++){
        ll curcost=0;
        vll understanding(m,0);
        // cerr << itobins(i) << endl;
        for(int j = 0; j < n; j++){
            if(i>>j&1){
                curcost+=c[n-1-j];
                fok(0,m){
                    understanding[k]+=a[n-1-j][k];
                }
            }
        }
        ll checker = 1;
        // cerr << understanding << endl;
        // cerr << curcost << endl;
        foj(0,m){
            if(understanding[j]<x) checker = 0;
        }
        // cerr << "checker: " << checker << endl;
        if(checker) ans=min(ans,curcost);
    }
    if(ans==INF){
        cout << -1 << endl;
        return 0;
