...
auto solve(int k, int q, const std::vector<long long> &d, const std::vector<long long> &n, const std::vector<long long> &x, const std::vector<long long> &m) {
...
int main() {
    ...
    std::cin >> q;
    std::vector<long long> n(q), x(q), m(q);
    REP (i, k) {
        std::cin >> d[i];
    }
    REP (i, q) {
