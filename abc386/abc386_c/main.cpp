...
int main(){
    // T has 1 character not in S
    // S has 1 character not in T
    // dif in length of S & T <=1
    // if S&T are of same length then only 1 character difference IN ORDER
    // if S&T differ in length by 1, remove 1 character systematically and check if ever equal
    ll K;
    cin >> K;
    string S,T;
    cin >> S >> T;
