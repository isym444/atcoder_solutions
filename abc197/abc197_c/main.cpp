...
int main(){
    ...
    for(int i = 0; i<(1<<N+2); i++){
        ...
        foj(0,N+1){
            ll cur = ti&1;
            ti=ti>>1;
            if(cur) divisions.pb(j);
        }
        ll l=0;
        ll txor=0;
        // foj(0,divisions.size()-1){
        if(divisions.size()<2) continue;
        if(divisions[0]!=0) continue;
        if(divisions[divisions.size()-1]!=N) continue;
        // dbg(mp(divisions[0],divisions[divisions.size()-1]));
        dbg(divisions);
        for(auto x: divisions){
            // dbg(j);
            ll tor=0;
            while(l!=x){
                tor=tor|A[l];
