...
long long solve(long long R, long long C, int K, const std::vector<long long> &r, const std::vector<long long> &c, const std::vector<long long> &v) {
...
int main() {
    ...
    int K;
    std::cin >> R >> C >> K;
    std::vector<long long> r(K), c(K), v(K);
    REP (i, K) {
        std::cin >> r[i] >> c[i] >> v[i];
    }
