...
int main() {
    ...
    auto dfs = [&](auto dfs, ll i, ll j, ll dir)->void{
        if(!isvalidsingle(i,0,8) || !isvalidsingle(j,0,8)){
            return;
        }
        // dbg("hello world");
        // visited[i][j]=1;
        ...
        if(dir==-1){
            fok(0,4){
                ...
                if (!isvalidsingle(ni, 0, 8) || !isvalidsingle(nj, 0, 8)) continue;
                // if(i+dx_wasd[k]==pi&&j+dy_wasd[k]==pj) continue;
                // dbg("hello");
            ...
            ll ni = i+dx_wasd[dir];
            ll nj = j+dy_wasd[dir];
            if (!isvalidsingle(ni, 0, 8) || !isvalidsingle(nj, 0, 8)) return;
