...
auto solve(long long N, int M, const std::vector<long long> &U, const std::vector<long long> &V, const std::vector<long long> &W, int Q, const std::vector<long long> &A, const std::vector<long long> &S, const std::vector<long long> &T) {
    // TODO: edit here
}
...
int main() {
    ...
    std::cin >> Q;
    std::vector<long long> A(Q), S(Q), T(Q);
    REP (i, Q) {
        std::cin >> A[i] >> S[i] >> T[i];
    }
    auto ans = solve(N, M, U, V, W, Q, A, S, T);
    REP (i, Q) {
        std::cout << a[i] << '\n';
    }
