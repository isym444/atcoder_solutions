...
ll stringToBinary(const std::string& s, char charAsOne) {
    ll x = 0;
...
int main() {
    ...
    foi(0,S.length()){
        if(ogs[i]=='?'){
            ...
            temp[i]='1';
            dbg(temp);
            if(stringToBinary(temp,'1')<=N){
                S=temp;
                ans=max(ans,(ll)stringToBinary(S,'1'));
            }
        }
    }
    if(stringToBinary(S,'1')<=N) ans = max(ans,(ll)stringToBinary(S,'1'));
    cout << ans << endl;
