...
vvll ans;
ll K;
void generateVectors(const vll& vector, vll& currentVector, int index) {
    if (index == vector.size()) {
        vll tv;
        ll ts=0;
        for (int num : currentVector) {
            tv.pb(num);
            ts+=num;
        }
        if(ts%K==0) ans.pb(tv);
    ...
    for (int i = 1; i <= vector[index]; ++i) {
        currentVector[index] = i;
        generateVectors(vector, currentVector, index + 1);
    }
}
