...
void solve(long long N, int T, const std::vector<long long> &A, const std::vector<long long> &B) {
    /* vis.assign(n+1, false);
    g.assign(n+1, vector<int>());
    wg.assign(n + 1, vector<pair<ll,ll>>());
    parent.assign(n+1, -1); */
    vll piscore(N+1,0);
    map<ll,ll> peopleatscore;
    peopleatscore[0]=N;
    foi(0,T){
        peopleatscore.find(piscore[A[i]])!=peopleatscore.end()&&peopleatscore[piscore[A[i]]]==1?peopleatscore.erase(piscore[A[i]]):peopleatscore[piscore[A[i]]]--;
        piscore[A[i]]+=B[i];
        //cerr << piscore << endl;
        peopleatscore.find(piscore[A[i]])!=peopleatscore.end()?peopleatscore[piscore[A[i]]]++:peopleatscore[piscore[A[i]]]=1;
        //cerr << peopleatscore << endl;
        cout << peopleatscore.size() << endl;
    }
