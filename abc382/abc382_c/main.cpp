...
int main(){
    ...
    // value of sushi -> which A eaten by;
    map<ll,ll> ans;
    sort(all(B));
    B.pb(INF);
    foi(0,N){
        ll cur = A[i];
        ll eatenby = indlb(B,cur);
        if(B[eatenby] == INF){
            // ans[B[eatenby]]=-1;
            continue;
        }
        if(ans.count(B[eatenby])!=0) continue;
        while(eatenby<M&&!ans.count(B[eatenby])){
            ans[B[eatenby]]=i+1;
            eatenby++;
            while(eatenby<M&&B[eatenby]==B[eatenby-1]) eatenby++;
        }
    }
    fx(Bog){
        if(!ans.count(x)){
    ...
    // }
    // // vp.pb(mp(INF,INF));
    // pair<ll,ll> lastBiggest = mp(-1,-1);
    // vector<pair<ll,ll>> finvp;
    // foi(0,N){
    //     auto [a,b] = vp[i];
    //     if(a<=lastBiggest.first){
    //         continue;
    //     }
    //     if(a>lastBiggest.first){
    //         lastBiggest=mp(a,b);
    ...
    // fx(B){
    //     // ll t = indlb(vp,x);
    //     auto it = lower_bound(finvp.begin(), finvp.end(), make_pair(x, LLONG_MIN),
