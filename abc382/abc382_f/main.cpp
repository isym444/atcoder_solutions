...
ll op(long f,long x){return max(f,x);}       // Defines the operation for the segment tree (sum operation).
ll e(){return -INF;}                      // Defines the identity element for the segment tree (0 for sum, 0 for xor, 0 for GCD, 1 for LCM, 1 for multiplication, INF for min, -INF for max).
...
int main() {
    ...
    int N;
    std::cin >> H >> W >> N;
    // std::vector<long long> R(N), C(N), L(N);
    vector<tuple<ll,ll,ll>> RCL;
    vector<ll>A(W+10);                     // Create a vector to hold the initial value at each element of segment tree.
    ...
    fx(RCL){
        // ?x+1
        ans[i]=seg.prod(get<1>(x),get<1>(x)+get<2>(x));
        i++;
        seg.apply(get<1>(x),get<1>(x)+get<2>(x),seg.prod(get<1>(x),get<1>(x)+get<2>(x))+1);
    }
    fx(ans){
        cout << x << endl;
    }
