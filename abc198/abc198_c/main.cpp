...
long long solve(long long R, long long X, long long Y) {
/* vis.assign(n+1, false);
g.assign(n+1, vector<int>());
wg.assign(n + 1, vector<pair<ll,ll>>());
parent.assign(n+1, -1); */
cerr << R << " " << X << " " << Y << endl;
cerr << sqrt(pow(X,2)+pow(Y,2))/R << endl;
cerr << ceildiv(sqrt(pow(X,2)+pow(Y,2)),(double)R) << endl;
ll temp = ceildiv(sqrt(pow(X,2)+pow(Y,2)),R);
if(!X){
    temp = ceildiv(Y,R);
    if(Y/R<1) return 2;
    return temp;
...
if(sqrt(pow(X,2)+pow(Y,2))/R<1) return 2;
return temp;
}
