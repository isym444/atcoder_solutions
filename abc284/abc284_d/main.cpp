...
vll GenListOfPrimesOnly(ll x){
    gen_primes();
    vll allprimes;
    for(int i = 0; i<MAXA; i++){
        if(prime[i]==1&&prime[i]<=x){
            allprimes.pb(i);
        }
    }
    return allprimes;
...
const int MAXN = 2 * 10000000 + 10; // Adjust the size as per the problem constraints
...
int main() {
    ...
    // spfsieve();
    vll p = GenListOfPrimesOnly(cbrt(maxnum));
    // cerr << p << endl;
    foi(0,t){
        foj(1,p.size()-1){
            if(a[i]%p[j]==0){
    ...
    // cerr << p[p.size()-1]*p[p.size()-1]*p[p.size()-2] << endl;
    // fx(a){
    //     auto y = fact(x);
    //     for(auto p:y){
    //         cout << p.first << " ";
