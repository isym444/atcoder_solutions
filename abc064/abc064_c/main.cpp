...
void solve(int N, const std::vector<long long> &a) {
    ...
    parent.assign(n+1, -1); */
    ll gray, brown, green, cyan, blue, yellow, orange, red;
    vll colors(9,0);
    foi(0,N){
        if(a[i]<400) colors[0]++;
        else if(a[i]<800) colors[1]++;
        else if(a[i]<1200) colors[2]++;
        else if(a[i]<1600) colors[3]++;
        else if(a[i]<2000) colors[4]++;
        else if(a[i]<2400) colors[5]++;
        else if(a[i]<2800) colors[6]++;
        else if(a[i]<3200) colors[7]++;
        else colors[8]++;
    }
    ll count=0;
    ll checker = 0;
