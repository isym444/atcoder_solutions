...
std::vector<auto> solve(int N, int M, const std::vector<long long> &A, const std::vector<long long> &L, const std::vector<long long> &R, const std::vector<long long> &X) {
    // TODO: edit here
}
...
int main() {
    ...
    std::cin >> M;
    std::vector<long long> L(M), R(M), X(M);
    REP (i, N) {
        std::cin >> A[i];
    }
    REP (i, M) {
