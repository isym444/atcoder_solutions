...
std::pair<long long, long long> solve(long long K, long long G, long long M) {
    // TODO: edit here
    //from 0:
    //if mug empty fill mug with water
    //transfer mug->glass until mug empty OR glass full
    //either fill mug if mug empty OR discard all water from glass and keep refilling from mug until mug empty
    ...
    K--;
        ...
        cerr << endl;
    while(K>0){
        //if can fill glass without emptying mug
        if(cg==G){
            cg=0;
            K--;
        }
        ...
        //if can fill glass but will empty mug
        else if(cm-(G-cg)==0){
            cg=G;
            cm=0;
            K--;
        }
        //if cannot fill glass with current mug water
        else{
            cg+=cm;
            cm=0;
            K--;
