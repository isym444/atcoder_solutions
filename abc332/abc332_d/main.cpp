...
int main(){
    ...
    queue<pair<vvll,ll>> q;
    q.push(mp(A,0));
    seen[A]=1;
    while(!q.empty()){
        vvll og = q.front().first;
        ...
        foi(0,W-1){
            vvll columnSwap = og;
            foj(0,H){
                swap(columnSwap[j][i],columnSwap[j][i+1]);
            }
            if(seen.count(columnSwap)) continue;
            q.push(mp(columnSwap,temp+1));
            seen[columnSwap]=1;
        }
        foi(0,H-1){
            vvll rowSwap = og;
            swap(rowSwap[i],rowSwap[i+1]);
            if(seen.count(rowSwap)) continue;
            q.push(mp(rowSwap,temp+1));
            seen[rowSwap]=1;
        }
    }
    cout << -1 << endl;
