...
auto solve(int N, const std::vector<long long> &A, const std::vector<long long> &B, const std::vector<long long> &C, const std::vector<long long> &D) {
...
int main() {
    ...
    int N;
    std::cin >> N;
    std::vector<long long> A(N - 1), B(N - 1), C(N - 1), D(N);
    REP (i, N - 1) {
        std::cin >> A[i] >> B[i] >> C[i];
    }
    REP (i, N) {
