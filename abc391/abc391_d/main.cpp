...
int main(){
    ...
    // heights, block number
    map<ll,set<pair<ll,ll>>> xtoheights;
    map<ll,set<ll>> timesetsnapshot;
    set<ll> blocksexisting;
    foi(1,N+1){
        blocksexisting.emplace(i);
    }
    ll cumtime = 0;
    ...
    foi(0,N){
        ll x,y;
        cin >> x >> y;
        xtoheights[x].emplace(mp(y,i+1));
    }

    vll erasedtimes;
    erasedtimes.pb(0);
    timesetsnapshot[0] = blocksexisting;
    ...
    // map<ll,ll> blockremovaltime;
    vector<ll> blockremovaltime(N+1,INF);

    while(xtoheights.size()>=W){
        ll maxminheight = -INF;
        for(auto &x:xtoheights){
            maxminheight = max(maxminheight,x.second.begin()->first);
        }
            // dbg(maxminheight);

        // cumtime+=maxminheight;

        // delete at maxminheight if full width remaining
        // dbg(xtoheights);
        // if(xtoheights.size()>=W){
        //     fx(xtoheights){
        //         blocksexisting.erase(x.second.begin());
        //         x.second.erase(x.second.begin());
        //         if(x.second.size()==0){
        //             xtoheights.erase(x.first);
        //         }
        //         dbg("reached");
        //     }
        // }
        ...
        auto it = xtoheights.begin();
        while (it != xtoheights.end()) {
            if (!it->second.empty()) {  // Check before accessing set.begin()
                auto smallest = it->second.begin();  // Store iterator before erasing
                blockremovaltime[smallest->second] = min(blockremovaltime[smallest->second],maxminheight);
                blocksexisting.erase(smallest->second);  // Erase value from blocksexisting (dereferencing)
                it->second.erase(smallest);       // Erase from set
            }

            // After removing an element, check if the set is now empty
            if (it->second.empty()) {
                it = xtoheights.erase(it);  // erase() returns next valid iterator
            } else {
                ++it;  // Move to the next entry in map
            }

            // dbg("reached");
        }
        // erasedtimes.pb(maxminheight);
        // timesetsnapshot[maxminheight] = blocksexisting;
    }
    // dbg(erasedtimes);
    // dbg(timesetsnapshot);
    dbg(xtoheights.size());
    ...
    cin >> Q;
    foi(1,N+1){
        if(blockremovaltime[i]==0){
            blockremovaltime[i] = INF;
        }
    }
    dbg(blockremovaltime);
    foi(0,Q){
        ll t,a;
        cin >> t >> a;
        // ll targtime = indub(erasedtimes,t);
        // targtime--;
        // auto queryset = timesetsnapshot[erasedtimes[targtime]];
        // dbg(queryset);
        if(blockremovaltime[a]>t){
            cout << "Yes" << endl;
        }else{
            cout << "No" << endl;
        }
