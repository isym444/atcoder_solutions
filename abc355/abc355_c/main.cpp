...
/* fill all out i.e. mark all squares
find if there are any bingos
one iteration for columns
one iteration for rows
2 iterations for diagonals
data structure: vvll initialized to 0
use N*(i)+(j) to mark squares
?keep priority queue of all max consecutives?
when fill in, keep the turn in which was filled in, when check for bingo, take min of squares that make bingo
 */
long long solve(long long N, int T, const std::vector<long long> &A) {
    /* vis.assign(n+1, false);
    g.assign(n+1, vector<ll>());
    wg.assign(n + 1, vector<pair<ll,ll>>());
    parent.assign(n+1, -1); */
    vvll board(N, vll(N,0));
    // vector<vector<pair<ll,ll>>(N,vector<pair<ll,ll>>)
    vvll instime(N, vll(N,-1));
    // dbg(board);
    fok(0, A.size()){
        ll x = A[k];
        x--;
    ...
    foi(0,N){
        ...
        foj(0,N){
            ...
            if(board[i][j]==1){
                checker++;
                // if(i==2) dbg(instime[i][j]);
                firstbingo=max(firstbingo,instime[i][j]);
            }
        }
        if(checker==N){
            // dbg(i);
            // return firstbingo;
            ans=min(ans,firstbingo);
        }
    }
    foj(0,N){
        ll checker = 0;
    ...
    foi(0,N){
        if(board[i][i]==1){
            checker++;
            firstbingo=max(firstbingo,instime[i][i]);
        }
    }
    if(checker==N){
        // return firstbingo;
    ...
    foi(0,N){
        if(board[N-1-i][i]==1){
            checker++;
            firstbingo=max(firstbingo,instime[N-1-i][i]);
        }
    }
    if(checker==N){
        // return firstbingo;
