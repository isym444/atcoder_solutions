...
int main(){
    ...
    //map mapping start to end of current segment
    dsu dd(N);

    //leader of connected component -> color of connected component
    map<ll,ll> m;

    //color -> count of that color
    map<ll,ll> m2;

    // leader of a connected component should be mapped to pair of beginning/end of that component
    ...
    foi(0,Q){
        ...
        if(a==1){
            ...
            //decrease count of xs current color by the size of xs connected component
            m2[m[dd.leader(x-1)]]-=dd.size(x-1);

            //change color of xs current connected component to c
            m[dd.leader(x-1)]=c-1;

            //increase count of xs current color by the size of xs connected component
            m2[m[dd.leader(x-1)]]+=dd.size(x-1);

            //join to adjacent components if same color
            ll indexbefore = m3[dd.leader(x-1)].first-1;
            ll indexafter = m3[dd.leader(x-1)].second+1;

            if(indexbefore>=0){
                //if color the same
                if(m[dd.leader(indexbefore)]==m[dd.leader(x-1)]){
                    ll temp = dd.leader(indexbefore);
                    ll temp2 = m3[temp].first;
                    ll temp3 = m3[dd.leader(x-1)].second;
                    //merge 2 components
                    dd.merge(indexbefore,x-1);
                    //update beginning and end indices of m3
                    m3[dd.leader(x-1)].first=temp2;
                    m3[dd.leader(x-1)].second=temp3;
                }
            }

            if(indexafter<N){
                //if color the same
                if(m[dd.leader(indexafter)]==m[dd.leader(x-1)]){
                    ll temp = dd.leader(indexafter);
                    ll temp2 = m3[temp].second;
                    ll temp3 = m3[dd.leader(x-1)].first;
                    //merge 2 components
                    dd.merge(indexafter,x-1);
                    //update beginning and end indices of m3
                    m3[dd.leader(x-1)].second=temp2;
                    m3[dd.leader(x-1)].first=temp3;
                }
            }

            // dd.merge(x-1,m2[c-1]);
            //leader of connected component -> color
            // m[dd.leader(x-1)]=dd.size(x-1);
            continue;
        }
        if(a==2){
            ll c;
