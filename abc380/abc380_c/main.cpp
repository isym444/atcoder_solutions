...
int main(){
    ...
    foi(0,N){
        auto cur = S[i];
        if(S[i]=='1'&&insideornot==0){
            m[block]=mp(i,-1);
            insideornot=1;
            if(i==N-1&&insideornot==1){
                m[block].second=i;
                continue;
            }
            continue;
    ...
    string ans;
    ll insertat = m[K-1].second+1;
    ll dontinsertfrom = m[K].first;
    ll dontinsertto = m[K].second;
    foi(0,N){
        if(i==insertat){
            ll temp=m[K].second-m[K].first+1;
            while(temp){
                ans+='1';
                temp--;
            }
...
// std::string solve(long long N, long long K, std::string S) {
