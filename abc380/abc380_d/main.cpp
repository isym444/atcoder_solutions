...
int main(){
    ...
    // ll ln = ST.length();
    // fx(K){
    //     char temp = ST[MOD(x-1,ln)];
    //     if(floordiv(x-1,ln)%2==1){
    //         dbg(temp);
    //         if(islower(temp)) temp=toupper(temp);
    //         else if(isupper(temp)) temp=tolower(temp);
    //         dbg(temp);
    //     }
    //     cout << temp << " ";
    // }
    fx(K){
        ll segment = floordiv(x-1,S.length());
        ll sett = setbits(segment);
        ll pos = MOD(x-1,S.length());
        if(sett%2==0){
            //then S
            cout << S[pos] << " ";
            continue;
...
// std::vector<auto> solve(std::string S, int Q, const std::vector<long long> &K) {
