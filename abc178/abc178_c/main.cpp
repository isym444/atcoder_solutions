...
template <int mod>
//h USE THIS over the atcoder library one above
//using mint = ModInt<1000000007>;
//constructor is used to declare variables of this data type e.g. mint(10)
struct ModInt
{
    ...
    //h note not a function unlike atcoder library
    int val;
    ...
    //h .power(x) to raise to the power of x e.g. mint(10).power(x)
...
long long solve(long long N) {
    mint a = mint(10).power(N);
    cerr << a.power(23);
    mint b = mint(9).power(N);
    mint c = mint(9).power(N);
    mint d = mint(8).power(N);
    cerr << a.val << endl;
    return mint(a-b-c+d).val;
}
