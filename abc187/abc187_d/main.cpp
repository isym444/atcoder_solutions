...
int main() {
    ...
    REP (i, N) {
        ...
        t = a+b;
        ll benefit=t+a;
        abt.pb({a,b,t,benefit});
    }
    sort(all(abt),[](tuple<ll,ll,ll,ll> aa, tuple<ll,ll,ll,ll> bb){
        //if total equal, sort first towns with more Aoki voters
        if(get<3>(aa) == get<3>(bb)){
            return get<2>(aa)>get<2>(bb);
        }
        else return get<3>(aa)>get<3>(bb);
    });
    // fx(abt){
