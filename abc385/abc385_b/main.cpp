...
int main(){
    ...
    if(G[cur.first][cur.second]=='@') ans.emplace(mp(X,Y));
    foi(0,T.size()){
        char dir = T[i];
        if(dir == 'L'){
            pair<ll,ll> ncur = mp(cur.first,cur.second-1);
            if(!iv(ncur.first,0,H)) continue;
            if(!iv(ncur.second,0,W)) continue;
            if(G[ncur.first][ncur.second]=='#') continue;
            cur=ncur;
            if(G[cur.first][cur.second]=='@') ans.emplace(mp(cur.first,cur.second));
        ...
        if(dir == 'U'){
            pair<ll,ll> ncur = mp(cur.first-1,cur.second);
            if(!iv(ncur.first,0,H)) continue;
            if(!iv(ncur.second,0,W)) continue;
            if(G[ncur.first][ncur.second]=='#') continue;
            cur=ncur;
    ...
    cout << cur.first+1 << " " << cur.second+1 << " " << ans.size() << endl;
