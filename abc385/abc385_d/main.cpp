...
int main(){
    ...
    map<ll,set<pair<ll,ll>>> tax;
    map<ll,set<pair<ll,ll>>> tay;
    ll curx,cury;
    foi(0,M){
        char d;
        ll c;
        ...
        if(d=='U'){
            ll starty = Y;
            Y+=c;
            tay[X].emplace(mp(starty,Y));
        }
        if(d=='D'){
            ll starty = Y;
            Y-=c;
            tay[X].emplace(mp(Y,starty));
        }
        if(d=='L'){
            ll startx = X;
            X-=c;
            tax[Y].emplace(mp(X,startx));
        }
        if(d=='R'){
            ll startx = X;
            X+=c;
            tax[Y].emplace(mp(startx,X));
        }
    }
    map<ll,set<pair<ll,ll>>> utax;
    map<ll,set<pair<ll,ll>>> utay;

    for(auto temp:tax){
        // dbg(mt(temp.first,temp.second));
        ll started = 0;
        ll curMax = -INF;
        ll curL = -INF;
        for(auto [l,r]:temp.second){
            // dbg(mp(l,r));
            if(l>curMax && started){
                utax[temp.first].emplace(mp(curL,curMax));
                curL=l;
                curMax=r;
                continue;
            }
    ...
    for(auto temp:tay){
        ...
        ll curMax = -INF;
        ll curLow = -INF;
        for(auto [low,high]:temp.second){
            // dbg(mp(l,r));
            if(low>curMax && started){
                utay[temp.first].emplace(mp(curLow,curMax));
                curLow=low;
                curMax=high;
                continue;
            }
            ...
            curMax=max(curMax,high);
        }
        utay[temp.first].emplace(mp(curLow,curMax));
    }

    // iterate through unique segments -> binary search to find L&R indexes of visited houses -> iterate through visited houses -> add these to set of visited houses
    set<pair<ll,ll>> visited;

    for(auto temp:utax){
        for(auto[l,r]:temp.second){
            vll vforbin(hay[temp.first].begin(),hay[temp.first].end());
            vforbin.pb(INF);
            ll lind = indlb(vforbin,l);
            ll rind = indub(vforbin,r);
            foi(lind,rind){
                // debug careful with ordering inside mp
                visited.emplace(mp(temp.first,vforbin[i]));
            }
        }
    }
    ...
    for(auto temp:utay){
        for(auto[low,high]:temp.second){
            vll vforbin(hax[temp.first].begin(),hax[temp.first].end());
            vforbin.pb(INF);
            ll lowerind = indlb(vforbin,low);
            ll upperind = indub(vforbin,high);
            foi(lowerind,upperind){
                // debug careful with ordering inside mp
                visited.emplace(mp(vforbin[i],temp.first));
            }
        }
    }
