...
void solve(std::string S, std::string T) {
    ...
    //#cs <= #ct
    //if #ct>1, #cs!=1
    vector<pair<char,ll>> sc;
    vector<pair<char,ll>> tc;
    ll p = 0;
    ll i = 1;
    ...
    while(p<S.size()){
        ll curcount=0;
        char curchar = S[p];
        while(S[p]==curchar&&p<S.size()){
            curcount++;
            p++;
        }
        sc.pb(mp(curchar,curcount));
    }
    p = 0;
    while(p<T.size()){
        ll curcount=0;
        char curchar = T[p];
        while(T[p]==curchar&&p<T.size()){
            curcount++;
            p++;
        }
        tc.pb(mp(curchar,curcount));
    }
    //cerr << sc << endl;
    //cerr << tc << endl;
    if(sc.size()!=tc.size()){
    ...
    foi(0,sc.size()){
        ...
        if(tc[i].second>1&&sc[i].second==1){
            cout << "No";
            return;
        }
    }
