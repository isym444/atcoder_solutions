...
long long solve(int N, const std::vector<long long> &d, const std::vector<long long> &u, const std::vector<long long> &v, const std::vector<long long> &w) {
    // TODO: edit here
}
...
int main() {
    ...
    int N;
    std::cin >> N;
    std::vector<long long> d(N), u(N - 1), v(N - 1), w(N - 1);
    REP (i, N) {
        std::cin >> d[i];
    }
    REP (i, N - 1) {
