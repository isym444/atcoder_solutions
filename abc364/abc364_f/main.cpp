...
long long solve(long long N, int Q, const std::vector<long long> &L, const std::vector<long long> &R, const std::vector<long long> &C) {
...
int main() {
    ...
    int Q;
    std::cin >> N >> Q;
    std::vector<long long> L(Q), R(Q), C(Q);
    REP (i, Q) {
        std::cin >> L[i] >> R[i] >> C[i];
    }
    auto ans = solve(N, Q, L, R, C);
    std::cout << ans << '\n';
