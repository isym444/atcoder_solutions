...
int main(){
    ...
    // map of number to i,j coordinate
    map<ll,pair<ll,ll>> m;
    foi(0,H){
        foj(0,W){
            cin >> A[i][j];
    ...
    auto f = [&](auto f, ll x)->ll{
        ...
        ll temp = abs(m[x].first-m[x-D].first)+abs(m[x].second-m[x-D].second);
        // dbg(temp);
        return memo[x] = temp + f(f,x-D);
    };
...
// pair<int, int> idx[90101]; // Stores coordinates of grid values
// int memo[90101];           // Memoization for f(x)

// int f(int x) {
//     if (memo[x]!=0) return memo[x]; // Return precomputed result
//     if (x < D) return 0;             // Base case: No movement

//     int y = x - D; // Previous value in sequence
//     int cost = abs(idx[x].first - idx[y].first) + abs(idx[x].second - idx[y].second);
//     int res = cost + f(y); // Calculate recursively
//     return memo[x] = res;  // Memoize result
// }

// void _main() {
//     cin >> H >> W >> D;

//     // Preprocess grid and store coordinates
//     rep(y, 0, H) rep(x, 0, W) {
//         cin >> A[y][x];
//         A[y][x]--; // Convert to 0-based indexing
//         idx[A[y][x]] = make_pair(x, y); // Store coordinates
//     }
...
//         int ans = f(R) - f(L); // Compute result for query
//         printf("%d\n", ans);   // Output result
//     }
// }
