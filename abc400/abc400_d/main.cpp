...
inline OStream &operator<<(OStream &os, const array<T, sz> &arr)
{
    os << '[';
    for (auto &v : arr)
        os << v << ',';
...
inline OStream &operator<<(OStream &os, const unordered_set<T, TH> &st)
{
    os << '{';
    for (auto &v : st)
        os << v << ',';
...
template <class OStream, class T>
inline OStream &operator<<(OStream &os, const deque<T> &dq)
{
    os << "deq[";
    for (auto &v : dq)
        os << v << ',';
...
template <class OStream, class T>
inline OStream &operator<<(OStream &os, const set<T> &st)
{
    os << '{';
    for (auto &v : st)
        os << v << ',';
...
template <class OStream, class T>
inline OStream &operator<<(OStream &os, const multiset<T> &st)
{
    os << '{';
    for (auto &v : st)
        os << v << ',';
...
template <class OStream, class T>
inline OStream &operator<<(OStream &os, const unordered_multiset<T> &st)
{
    os << '{';
    for (auto &v : st)
        os << v << ',';
...
inline OStream &operator<<(OStream &os, const pair<T, U> &pa) { return os << '(' << pa.first << ',' << pa.second << ')'; }
template <class OStream, class TK, class TV>
inline OStream &operator<<(OStream &os, const map<TK, TV> &mp)
{
    os << '{';
    for (auto &m : mp)
        os << m.first << "=>" << m.second << ',';
...
inline OStream &operator<<(OStream &os, const unordered_map<TK, TV, TH> &mp)
{
    os << '{';
    for (auto &m : mp)
        os << m.first << "=>" << m.second << ',';
...
template <class OStream, class T, size_t rows, size_t cols>
inline OStream &operator<<(OStream &os, T (&arr)[rows][cols])
{
    os << '[';
...
template <class... T>
inline istream &operator>>(istream &is, tuple<T...> &tpl)
{
...
template <class OStream, class... T>
inline OStream &operator<<(OStream &os, const tuple<T...> &tpl)
{
    os << '(';
...
template <class T>
int indlb(const std::vector<T> &v, const T &x) { return std::distance(v.begin(), std::lower_bound(v.begin(), v.end(), x)); }
...
template <class T>
int indub(const std::vector<T> &v, const T &x) { return std::distance(v.begin(), std::upper_bound(v.begin(), v.end(), x)); }
...
#define iv(value, min_valid_value, max_valid_value) (min_valid_value <= value and value < max_valid_value)
    ...
    while (!pq.empty())
    {
        ...
        foi(0, 4)
        {
            ...
            if (visited[nx][ny])
                continue;
            // visited[nx][ny] = 1;
            pq.push({dist + distadder, nx, ny});
            nx += dx[i];
            ny += dy[i];
            if (nx < 0 || nx >= H || ny < 0 || ny >= W)
                continue;
            if (S[nx][ny] == '#')
                distadder = distadder | 1;
            if (visited[nx][ny])
                continue;
            // visited[nx][ny] = 1;
            pq.push({dist + distadder, nx, ny});
