...
ll crev(ll i){
    ll mov = i%10;
    i/=10;
    ll numdigits = to_string(i).length();
    i+=(mov*pow(10,numdigits));
    return i;
}
...
long long solve(long long a, long long N) {
    ...
    ll mm = (ll)1e6;
    // g.resize(mm);
    g.assign(mm+1, vector<ll>());
    vis.assign(mm+1, false);
    parent.assign(mm+1, -1);
    // dbg(g);
    foi(0,mm){
        if(i*a<=mm){
            // g[i].pb(i*a);
    ...
    // dbg(g);
    dbg(bfs(1,N));
    if(bfs(1,N).size()-1==0) return -1;
    return bfs(1,N).size()-1;
}
