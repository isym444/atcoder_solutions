...
int main(){
    ...
    foi(0,S.length()+1){
        if(i==0 || dp[i]){
            if(i+4<S.length() && (S.substr(i,5)=="dream" || S.substr(i,5)=="erase")) dp[i+5]=1;
            if(i+5<S.length() && S.substr(i,6)=="eraser") dp[i+6]=1;
            if(i+6<S.length() && S.substr(i,7)=="dreamer") dp[i+7]=1;
        }
    }
    dbg(dp);
    if(dp[S.length()]){
