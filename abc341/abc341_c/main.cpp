...
long long solve(int H, long long W, long long N, std::string T, std::vector<std::string> &S) {
    /* for(auto h:S) cerr << h << endl; */
    ll count = 0;
    for(int i = 1; i<H-1;i++){
        for(int j = 1; j<W-1;j++){
            if(S[i][j]=='.'){
