...
auto solve(int N, long long X, const std::vector<long long> &P, const std::vector<long long> &B, const std::vector<long long> &W, const std::vector<long long> &C) {
...
int main() {
    ...
    long long X;
    std::cin >> N;
    std::vector<long long> P(N - 1), B(N), W(N), C(N);
    std::cin >> X;
    REP (i, N - 1) {
        std::cin >> P[i];
    }
