...
int main(){
    ...
    dbg(mt(S,T));
    vvll hor(H,vll(W,INF));
    vvll ver(H,vll(W,INF));
    hor[S.first][S.second]=0;
    ver[S.first][S.second]=0;
    queue<pll> q;
    // coordinates to distance travelled
    // map<pair<ll,ll>,ll> ctod;
    // ctod[S]=0;
    q.emplace(S);
    // visited[S.first][s.second]=1;
    // ?dp where you do bfs but both for prev move being vertical and prev move being horizontal?
    while(!q.empty()){
        auto [f,s]=q.front();
        q.pop();
        // visited[f][s]=1;
        foi(0,4){
            ...
            ni=f+dx[i];
            nj=s+dy[i];
            if(!iv(ni,0,H)) continue;
            if(!iv(nj,0,W)) continue;
            // if(visited[ni][nj]) continue;
            if(G[ni][nj]=='#') continue;
            // if(G[ni][nj]=='G'){
            //     cout << ctod[mp(f,s)]+1 << endl;
            //     return 0;
            // }
            // q.emplace(mp(ni,nj));
            if(i==0||i==1){
                // vertical
                ll cur = ver[ni][nj];
                ver[ni][nj]=min(ver[ni][nj],hor[f][s]+1);
                if(hor[f][s]+1<cur){
                    q.emplace(mp(ni,nj));
                }
            }
            if(i==2||i==3){
                // horizontal
                ll cur = hor[ni][nj];
                hor[ni][nj]=min(hor[ni][nj],ver[f][s]+1);
                if(ver[f][s]+1<cur){
                    q.emplace(mp(ni,nj));
                }
            }
            // ctod[mp(ni,nj)]=ctod[mp(f,s)]+1;
        }
    }
    if(min(hor[T.first][T.second],ver[T.first][T.second])==INF){
        cout << -1 << endl;
        return 0;
    }
    cout << min(hor[T.first][T.second],ver[T.first][T.second]) << endl;
    return 0;
}