...
int main(){
    ll A,B,C;
    // # squares, # numbers on dice, # moves to target
    cin >> A >> B >> C;
    mint ans=0;
    // dp[i][j] where i = turn, j = square on
    vector<vector<mint>> dp(1005, vector<mint>(1005,0));
    dp[0][0]=1;
    foi(0,C){
        foj(0,A){
            fok(1,B+1){
                ...
                // if(nj>A) nj=A-(k-(A-j));
                if(nj>A) nj=A-(nj-A);
                if(nj==A) ans+=dp[i][j]/B;
                if(nj!=A) dp[i+1][nj]+=dp[i][j]/B;
            }
        }
    }
    cout << ans.val() << endl;
