...
long long solve(long long N, long long K, long long R, long long S, long long P, std::string T) {
    /* vis.assign(n+1, false);
    g.assign(n+1, vector<ll>());
    wg.assign(n + 1, vector<pair<ll,ll>>());
    parent.assign(n+1, -1); */
    vector<vector<ll>> dp(3,vll(N,0));
    foi(0,N){
        foj(0,3){
            char compmove = T[i];
            char usermove;
            ...
            if(j==2) usermove = 'p';
            ll roundscore = 0;
            if(usermove == 'r' && compmove == 's') roundscore = R;
            if(usermove == 's' && compmove == 'p') roundscore = S;
            if(usermove == 'p' && compmove == 'r') roundscore = P;
            // dbg(i);
            // dbg(j);
            // dbg(roundscore);
            if(dp[j][i]!=-INF) dp[j][i]+=roundscore;
            if(j==2){
                ll temp = max({dp[0][i],dp[1][i],dp[2][i]});
                ll counter = 0;
                ll totransfer = -1;
                fok(0,3){
                    if(dp[k][i]==temp){
