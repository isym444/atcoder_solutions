...
long long solve(long long a, int b, long long c, const std::vector<long long> &d, const std::vector<std::vector<long long> > &e, const std::vector<char> &f) {
...
int main() {
    ...
    for (int i = 0; i < totalCombinations; ++i) {
        //debug remove this line after testing
        std::vector<int> subsettest;
        std::vector<int> subset(n, 0);
        ll subsetcount=0;
        for (int j = 0; j < n; ++j) {
            // Check if the j-th element is included in the i-th subset
            if (i & (1 << j)) {
                subsettest.push_back(numbers[j]);
                subsetcount++;
                subset[j] = 1;
            }
        }
        if(subsetcount>=0){
            //combinations of real keys
            dbg(subset);
            dbg(subsettest);
            ll checker = 1;
            //check doesn't contradict any of the tests. If does, checker=0 & break.
            for(int ii=0; ii<m; ii++){
                if(R[ii]==1){
                    ...
                    //if keys mentioned in true combo that are actually valid keys < k -> break
                    // if((counter<k)){
                    if(C[ii]-counter<k){
                        checker=0;
                        break;
                    ...
                    fx(A[ii]){
                        ll t2=x-1;
                        if(subset[t2]==1) counter++;
                    }
                    //if keys mentioned in false combo are actually valid keys >= k -> break
                    // if((counter>=k)){
                    if(counter>=k){
                        checker=0;
                        break;
