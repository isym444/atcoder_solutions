...
// void solve(int N, std::vector<long long> &L, std::vector<long long> &R) {
...
int main() {
    ...
    cerr << rs << endl;
    cerr << rs.number_of_ranges() << endl;
    cerr << rs.contains(30) << endl;
    auto rr = rs.find_range(30);
    if(rr!=rs.end()){
        cerr << (rr->first) << "..." << (rr->second) << endl;
    }
    cerr << "min excluded: " << rs.minimum_excluded(20) << endl;
    cerr << rs.number_of_elements() << endl;
    cerr << rs.size() << endl;
    return 0;
}
