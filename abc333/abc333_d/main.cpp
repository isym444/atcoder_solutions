...
void bfs(ll u)
{
    ...
    q.push(u);
    vis[u] = true;
    depth[u] = 1; // Depth of starting node is 0
...
void dfsSubtrees(ll startNode){
    ...
    for(auto adjNode : g[startNode]){
        ...
        minans=min(minans,subtreeSizes[adjNode]);
    }
}

long long solve(int N, std::vector<long long> &u, std::vector<long long> &v) {
    /* cerr << u << endl;
    cerr << v << endl; */
    ...
    if(g[1].size()==1) return 1;
        ...
        ll total = 0;
        ll longestSubtree = 0;
        for(auto x:subtreeSizes){
            //cerr << x << endl;
            total += x.second;
            longestSubtree=max(longestSubtree,x.second);
        }
        //sum of all subtrees-longest subtree+1
        return total-longestSubtree+1;
    }
}
