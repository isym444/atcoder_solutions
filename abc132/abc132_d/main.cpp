...
// // Computes the number of ways to distribute n indistinct items into k distinct boxes,
// // allowing empty boxes (stars and bars theorem)
// mint f2(int n, int k) {
...
// mint f(int n, int k) {
//     if (n < k) return 0; // Not enough items to fill each box
//     if (n == 0 && k == 0) return 1; // Only one way to distribute zero items into zero boxes
//     if (k < 1) return 0; // Cannot distribute into less than one box
//     // Adjust 'n' and 'k' to account for the constraint of at least one item per box
//     // Subtract one item per box, reducing the problem to distributing 'n - k' items into 'k' boxes (which already have at least 1 item)
//     return f2(n-k,k);
//     // equivalent:
