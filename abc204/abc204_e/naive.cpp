...
long long solve(long long N, int M, const std::vector<long long> &A, const std::vector<long long> &B, const std::vector<long long> &C, const std::vector<long long> &D) {
    // TODO: edit here
}
...
int main() {
    ...
    int M;
    std::cin >> N >> M;
    std::vector<long long> A(M), B(M), C(M), D(M);
    REP (i, M) {
