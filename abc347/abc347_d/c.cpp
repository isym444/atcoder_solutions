...
int main() {
    ...
    foi(0,t){
        ...
        foj(0,x){
            cin >> bessiechoices[j];
            minchoice=min(minchoice,bessiechoices[j]);
            maxchoice=max(maxchoice,bessiechoices[j]);
        }
        sort(all(bessiechoices));
        if(bessiechoices[0]!=1){
            modifier=bessiechoices[0]-1;
        }
        foj(0,x){
            bessiechoices[j]=bessiechoices[j]-modifier;
        }
        minchoice-=modifier;
        maxchoice-=modifier;
        if(x==0||x==1){
        ...
        // cerr << bessiechoices << endl;
        // cerr << minchoice << endl;
        // cerr << maxchoice << endl;
        ll ff = bessiechoices[0];
        ll ans=0;
        ll hasafter=0;
        ll hasbefore=0;
        // if(bessiechoices[1]-bessiechoices[0]==1){
        //     //todo
        // }
        // if(bessiechoices[0]==1&&bessiechoices[x-1]==n){
        //     //todo
        // }
        fx(bessiechoices){
            // if(abs(x-ff)>1&&abs(x-ff)<n-1) ans++;
            if(x!=1&&x!=2&&x!=n) ans++;
            if(x==3) ans++;
            if(x==n-1) ans++;
        }
        if(ans>0) ans--;
        foj(0,x-1){
            if(bessiechoices[j+1]-bessiechoices[j]==2) ans++;
            if(bessiechoices[j+1]>=4&&bessiechoices[j]==2) ans++;
            if(bessiechoices[j+1]==n&&bessiechoices[j]<=n-2) ans++;
        }
        // cerr << "here" << ans << endl;
        // if(find(bessiechoices.begin(),bessiechoices.end(),3)!=bessiechoices.end()) ans++;
        // if(find(bessiechoices.begin(),bessiechoices.end(),n-1)!=bessiechoices.end()) ans++;
        // if(maxchoice==n-1) ans++;
        //h potential error with small numbers
        // if(bessiechoices[1]==2&&bessiechoices[2]==4) ans+=2;
        // if(bessiechoices[1]==2&&bessiechoices[2]>4) ans++;
        // if(maxchoice==n&&bessiechoices[x-2]==n-2) ans+=2;
        // if(maxchoice==n&&bessiechoices[x-2]<n-2) ans++;
        cout << ans << endl;
    }
