...
bool detectlegal(ll testing, ll A, ll B){
    // cerr << "test: " << testing << " " << A+B << endl;
    // cerr << MOD(2,7) << endl;
    ll st = MOD(testing, (A+B));
    // cerr << "st: " << st << endl;
    if(st==0) st=A+B;
    // cerr << "test:" <<MOD(2,7) << endl;
    if(st>A) return false;
    return true;
}

bool solve(int N, long long A, long long B, const std::vector<long long> &D) {
    /* vis.assign(n+1, false);
    g.assign(n+1, vector<ll>());
    wg.assign(n + 1, vector<pair<ll,ll>>());
    parent.assign(n+1, -1); */
    //calculate all the differences first
    //for each difference, find lower and upper bound where you can jump to
    //for each subsequent difference, find whether still land in AC testing upper and lower bound of previouso ne
    foi(0,N-1){
        difs.pb(D[i+1]-D[i]);
    }
    // cerr << difs << endl;
    ...
    foi(0, difs.size()){
        ...
        curlb=prevlb+curdif;
        curub=prevub+curdif;
        // cerr << curlb << " " << curub << " " << endl;
        // cerr << detectlegal(curlb, A, B) << endl;
        // cerr << detectlegal(curub, A, B) << endl;
        if(detectlegal(curlb, A, B)==0&&detectlegal(curub, A, B)==0) return false;
        // if(true) cerr << "reached" << endl;
        if(detectlegal(curlb, A,B)==0){
            ll diff;
            ll curubsmall=MOD(curub,(A+B));
            if(curubsmall==0) curubsmall=A+B;
            diff=curubsmall-1;
            curlb=curub-diff;
        }
        else if(detectlegal(curub, A,B)==0){
            ll diff;
            ll curlbsmall=MOD(curlb,(A+B));
            if(curlbsmall==0) curlbsmall=A+B;
            diff=A-curlbsmall;
            curub=curlb+diff;
        }
        // cerr << curlb << " " << curub << " " << endl;
