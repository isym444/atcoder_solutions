...
//fills the "prime" array so that when you query it with a number, it tells you if it's prime or not
void gen_primes() {
...
long long solve(int N, const std::vector<long long> &A) {
    ...
    ans+=zeros*nonzeros;
    ans+=nC2(zeros);
    //cerr << "ans after dealing with zeros: " << ans << endl;
    map<int,int> prevoddfactorization;
    foi(0,N){
        if(A[i]!=0){
            ll temp = 1;
            //cerr << A[i] << ": " << fact(A[i]) << endl;
            for(auto x:fact(A[i])){
                if(x.second%2){
                    temp*=x.first;
                }
            ...
            //cerr << "temp: " << temp << endl;
            if(prevoddfactorization.find(temp)!=prevoddfactorization.end()){
                ans+=prevoddfactorization[temp];
                prevoddfactorization[temp]++;
            }
            else prevoddfactorization[temp]=1;
            }
            /* cerr << "cur ans: " << ans << endl;
            cerr << prevoddfactorization << endl; */
    }
    return ans;
