...
long long solve(long long N, int K, std::vector<long long> &A) {
    cerr << A;
    vector<ll> vv;
    vv.pb(0);
    ll weird=0;
    ...
    cerr << vv << endl;
    if((vv.size()-1)%2==0){
        for(int i = 1; i<(vv.size()-1); i+=2){
            weird+=(vv[i+1]-vv[i]);
        }
    }
    else{
        for(int i = 2; i<(vv.size()-1); i+=2){
            weird+=(vv[i+1]-vv[i]);
        }
        cerr << weird << endl;
        ll potential=weird;
        for(int i = 2; i<(vv.size()-1); i+=2){
            potential = potential+((vv[i]-vv[i-1])-(vv[i+1]-vv[i]));
            /* cerr << endl;
            cerr << vv[i]-vv[i-1] << endl;
            cerr << vv[i+1]-vv[i] << endl;
            cerr << potential << endl;
            cerr << endl; */
            weird = min(weird, potential);
        }
    }
    //cerr << vv.size() << endl;
    return weird;
