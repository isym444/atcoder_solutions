...
long long solve(int N, long long A, const std::vector<long long> &W, const std::vector<long long> &X, const std::vector<long long> &V) {
...
int main() {
    ...
    long long A;
    std::cin >> N;
    std::vector<long long> W(N), X(N), V(N);
    std::cin >> A;
    REP (i, N) {
        std::cin >> W[i] >> X[i] >> V[i];
    }
