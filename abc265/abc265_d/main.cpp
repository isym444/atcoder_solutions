...
bool solve(int N, long long P, long long Q, long long R, const std::vector<long long> &A) {
    ...
    dbg(cum);
    for(auto i : cc){
        if(find(all(cc),(P+i))!=cc.end()&&find(all(cc),(Q+P+i))!=cc.end()&&find(all(cc),(R+Q+P+i))!=cc.end()){
            return true;
        }
        // if(cc.find(i+P)!=cc.end()&&cc.find(i+P+Q)!=cc.end()&&cc.find(i+P+Q+R)!=cc.end()){
        //     return true;
        // }
    }
    return false;
