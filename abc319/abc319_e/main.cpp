...
auto solve(int N, long long X, long long Y, const std::vector<long long> &P, const std::vector<long long> &T, int Q, const std::vector<long long> &q) {
...
int main() {
    ...
    int Q;
    std::cin >> N;
    std::vector<long long> P(N - 1), T(N - 1);
    std::cin >> X >> Y;
    REP (i, N - 1) {
        std::cin >> P[i] >> T[i];
    }
