...
bool isLexicographicallySmaller(const std::vector<int>& vec1, const std::vector<int>& vec2) {
...
ll ans2=0;
void dfs(const vector<vector<ll>> &a, ll x, ll y, const string &ansss, ll counter, ll n){
    ll q = a[x][y];
    ll w = ansss[counter]-'0';
    if((ll) a[x][y]==(ll) ansss[counter]-'0'){
        if(counter==n){
            /* cerr << "q: " << q << endl;
            cerr << "w: " << w << endl; */
            ans2++;
        ...
        if(x==0){
            if(y<n-1) dfs(a, 0, y+1, ansss, counter+1,n);
            dfs(a, 1, y, ansss, counter+1,n);
        }
        if(x==1){
            dfs(a,1,y+1, ansss, counter+1,n);
        }
    }
    else return;
}
