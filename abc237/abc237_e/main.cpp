...
int main(){
    ...
    vector<vector<pair<ll,ll>>> cpg(N);

    foi(0,N){
        fx(g[i]){
            cpg[i].pb(mp(x,max((ll)0,H[x]-H[i])));
        }
    }
    ...
    priority_queue<P, vector<P>, greater<P>> pq; //distance, from start to which node
    ...
    while(!empty(pq)){
        ...
        for(auto x : cpg[v]){
            ...
            distance[u]=min(distance[u],d+cost);
            pq.push(mp(distance[u],u));
        }
    }
    ...
    ll ans=0;
    foi(0,N){
        ans=max(ans,H[0]-H[i]-distance[i]);
    }
    cout << ans << endl;
