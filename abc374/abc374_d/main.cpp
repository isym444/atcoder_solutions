...
auto ceildiv(T n, U d) -> decltype(n / d + 0) {
    ...
    if constexpr (std::is_floating_point<T>::value || std::is_floating_point<U>::value) {
        ...
        return static_cast<decltype(n / d + 0)>(std::ceil(n / static_cast<long double>(d)));
    } else {
...
auto floordiv(T n, U d) -> decltype(n / d + 0) {
    ...
    if constexpr (std::is_floating_point<T>::value || std::is_floating_point<U>::value) {
        ...
        return static_cast<decltype(n / d + 0)>(std::floor(n / static_cast<long double>(d)));
    } else {
        // Handle case where both n and d are integers
...
int main() {
    ...
    cout << fixed << setprecision(28);
    unordered_map<long long, int, custom_hash> safe_map;
    int N;
    long double S, T;
    std::cin >> N;
    std::vector<long double> A(N), B(N), C(N), D(N);
    std::cin >> S >> T;
    REP (i, N) {
    ...
    foi(0,N){
        // segtimes[i]=sqrtl(((long double)abs(C[i]-A[i]))*(abs(C[i]-A[i]))+((long double)abs(D[i]-B[i]))*(abs(D[i]-B[i])))/(long double)T;
    ...
    //if start = 0 then AB, if start = 1 then CD
    auto dfs = [&](auto dfs, ll parent, ll parentStart, long double totaltime, ll count, vll visited)->void{
        if(count==N){
            // dbg(totaltime);
        ...
        foi(0,N){
            ...
            visited[i]=1;
            //travel0 is travel to new selection start, travel1 is travel to new selection end
            long double travel0=0;
            long double travel1=0;
            // travel0=sqrtl(((long double)abs(A[i]-parentx))*(abs(A[i]-parentx))+((long double)abs(B[i]-parenty))*(abs(B[i]-parenty)))/(long double)S;
            travel0=hypot(A[i]-parentx,B[i]-parenty)/S;
            // travel1=sqrtl(((long double)abs(C[i]-parentx))*(abs(C[i]-parentx))+((long double)abs(D[i]-parenty))*(abs(D[i]-parenty)))/(long double)S;
            travel1=hypot(C[i]-parentx,D[i]-parenty)/S;
            dfs(dfs, i, 1,segtimes[i]+totaltime+travel0,count+1, visited);
            dfs(dfs, i, 0,segtimes[i]+totaltime+travel1,count+1, visited);
            visited[i]=0;
        }
    ...
    //     // vll vist(N,0);
    //     visited[i]=1;
    //     long double temp1 = hypot(0-A[i],0-B[i])/S;
    //     long double temp2 = hypot(0-C[i],0-D[i])/S;
    //     dfs(dfs, i, 0, segtimes[i]+temp1,1);
    //     dfs(dfs, i, 1, segtimes[i]+temp2,1);
    //     visited[i]=0;
    // }
    cout << ans << endl;
