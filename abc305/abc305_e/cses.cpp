...
public:
    ...
    std::vector<std::vector<int>> groups()
    {
        ...
        result.erase(
            std::remove_if(result.begin(), result.end(), [&](const std::vector<int> &v)
                           { return v.empty(); }),
            result.end());
        return result;
    }
...
std::vector<Seq> split(const Seq s, typename Seq::value_type delim)
{
    ...
    {
        while (itr != std::end(s) and *itr != delim)
            ++itr;
...
int main()
{
    ...
    vll dist(N, INF);
    // priority_queue<pair<ll,ll>, vector<pair<ll,ll>>, function<bool(pair<ll,ll>, pair<ll,ll>)>> pq([](pair<ll,ll> a, pair<ll,ll> b){return a.first > b.first;});
    priority_queue<pair<ll, ll>, vector<pair<ll, ll>>, greater<pair<ll, ll>>> pq;
    pq.push(mp(0, 0));
    // dbg(wg);
    dist[0] = 0;
    while (!pq.empty())
    {
        ...
        fx(wg[v])
        {
            ...
            pq.emplace(mp(x.second + d, x.first));
        }
    }
    fx(dist)
    {
