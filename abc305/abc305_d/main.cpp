...
int main() {
    ...
    // cerr << "sleepsofar: " << sleepsofar << endl;
    REP (i, N) {
        std::cin >> A[i];
        if(i%2==0){
            if(i!=0) sleepsofar[i]=sleepsofar[i-1]+(A[i]-A[i-1]);
        }
        else{
            sleepsofar[i]=sleepsofar[i-1];
        }
    }
    // cerr << "sleepsofar: " << sleepsofar << endl;
    std::cin >> Q;
    ...
    REP (i, Q) {
        ...
        cin >> lb >> rb;
        ll llb, rrb;
        llb = lower_bound(all(A),lb)-A.begin();
        if(A[llb]!=lb){
            llb--;
        }
        rrb=lower_bound(all(A),rb)-A.begin();
        if(A[rrb]!=rb){
            rrb--;
        }
        ll al;
        ...
        ll ar=sleepsofar[rrb];
        if(rrb%2==1){
            ar+=rb-A[rrb];
        }
        // cerr << A[llb] << " " << A[rrb] << endl;
        // cerr << al << " " << ar << endl;
        // cerr << endl;
        cout << ar-al << endl;
    }
