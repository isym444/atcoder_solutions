...
long long solve(int N, std::vector<long long> &L) {
    sort(all(L));
    dbg(L);
    ll ans=0;
    foi(0,N){
        foj(i+1,N){
            if(i==j) continue;
            ll clb, cub;
            clb=max((ll)max(lower_bound(all(L),L[i]-L[j])-L.begin(),lower_bound(all(L),L[j]-L[i])-L.begin()),(ll)j+1);
            // cerr << L[i] << " " << L[j] << " " << L[c] << endl;
            cub=lower_bound(all(L),L[i]+L[j])-L.begin();
            dbg(clb);
            dbg(cub);
            ll tsum=0;
            tsum=cub-clb;
