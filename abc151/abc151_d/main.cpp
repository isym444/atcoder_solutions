...
int main() {
    ...
    vvll usedAsInput(H,vll(W,0));

    ll ans=0;

    vector<int> dx_wasd = {1,-1,0,0};
    vector<int> dy_wasd = {0,0,1,-1};
    vvll visog(H,vll(W,0));
    auto bfs = [&](int i, int j)->tuple<ll,ll,ll>{
        vvll visited(H,vll(W,0));
        queue<tuple<ll,ll,ll>> q;
        ll dist = 0;
        tuple<ll,ll,ll> dist_node = mt(0,-1,-1);
        q.push(mt(i,j,dist));
        visited[i][j]=1;
        visog[i][j]=1;
        // if(i==k && j==l) return(dist);
        while(!q.empty()){
            auto [curi, curj, newdist] = q.front();
            dist=max(dist,newdist);
            if(get<0>(dist_node)<newdist){
                dist_node=mt(newdist,curi,curj);
            }
            q.pop();
            // dbg(mp(curi,curj));
            // dbg("hello");
            for(int ii=0; ii<4; ii++){
                // dbg("hello2");
                ll newi = curi+dy_wasd[ii];
                ll newj = curj+dx_wasd[ii];
                if(!isvalidsingle(newi,0,H)) continue;
                if(!isvalidsingle(newj,0,W)) continue;
                if(visited[newi][newj]) continue;
                if(S[newi][newj]=='#') continue;
                dbg(mp(newi,newj));
                visited[newi][newj]=1;
                visog[newi][newj]=1;
                q.push(mt(newi, newj, newdist+1));
            }
        }
        return dist_node;
        // return (ll)1234;
    ...
    foi(0,H){
        foj(0,W){
            ...
            auto temp = bfs(i,j);
            dbg(" ");
            auto temp2 = bfs(get<1>(temp),get<2>(temp));
            ans=max(ans,get<0>(temp2));
            if(visog[i][j]) continue;
        }
    }
