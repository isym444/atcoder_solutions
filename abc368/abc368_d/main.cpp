...
int main()
{
    ...
    //whether a vertex MUST be included
    vll V(2e5 + 9, 0);
    //vertex to start dfs at
    ll starting=-1;
    foi(0, K)
    {
        ll t;
...
//         int x;
//         cin >> x;
//         V.insert(x - 1); // 0-indexing
//     }

//     // Initializing degrees and queue for leaves
//     vector<int> deg(N);
//     queue<int> q;

//     for (int i = 0; i < N; ++i) {
//         deg[i] = edge[i].size();
//         if (deg[i] == 1) q.push(i); // If it's a leaf node
//     }
...
//         int vv = *edge[v].begin(); // The only neighbor of the leaf v
//         edge[v].erase(vv);
//         edge[vv].erase(v);
//         ans--;

//         if (edge[vv].size() == 1) q.push(vv); // If vv becomes a leaf
//     }
