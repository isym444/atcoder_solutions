...
int main(){
    ...
    foi(0,M){
        ...
        b--;
        if(!dd.same(a,b)){
            // ll alead = dd.leader(a);
            // ll blead = dd.leader(b);
            // queue<pair<ll,ll>> tq = mp[alead];
            // fx(mp[blead]){
            //     tq.push(x);
            // }
            dd.merge(a,b);
            // ll newlead = dd.leader(a);
            // mp[newlead]=tq;
        } else{
            q.push({i,b});
        }
    ...
    if(dd.groups().size()==1){
        ...
        multiset<ll> ms;
        fx(dd.groups()){
            // q2.push_back(dd.leader(x[0]));
            ms.emplace(dd.leader(x[0]));
        }
        cout << ms.size()-1 << endl;
        while(ms.size()>1){
            auto [fff,sss] = q.front();
            q.pop();
            //q2.front() might not be the leader
            ll tojoin=-1;
            ...
            cout << fff+1 << " " << sss+1 << " " << tojoin+1 << endl;

            // ((q2.front()==dd.leader(sss))?(q2.pop_back()):(q2.pop_front()));
        }
        // foi(0,dd.groups().size()-1){
        //     mp[dd.leader(dd.groups()[i])]
        // }
    }
    return 0;
}