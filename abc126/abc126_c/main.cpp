...
double solve(long long N, long long K) {
    ...
    double ans=0;
    foi(0,N){
        double diescore=i+1.0;
        double temp=ceil(log2(K/diescore)); //h potential bug with /
        if(temp<0) temp=0;
        // cerr << temp << endl;
        // cerr << pow(0.5,temp) << endl;
        // cerr << (1.0/N)*pow((long double) 0.5, temp) << endl;
        // cerr << (1.0/N)*exp(temp * log(0.5)) << endl;
        // double temp2 = exp(temp*log(0.5));
        // cerr << temp << endl;
        double temp2 = pow(0.5, temp);
        // cerr << (1.0/N)*temp2 << endl;
        ans+=(1.0/N)*temp2;
    }
    return ans;
}
